...
Lean and Full extract the same supported facts and events. Use **Full** when a model benefits from the expanded field guide or richer optional metadata; Lean keeps the same content coverage in a simpler format.
...
- **State Spec Verbosity: Lean** and **Full** both audit every schema family, retain every supported durable change, and receive the same Engine Pass completion budget. Lean simplifies formatting, note length, and optional descriptive metadata; Full supplies the expanded field guide.
...
| `living_world` — Living World | **Active** | **Off:** render only on-page life. **Minimal:** off-screen time leaves evidence but no independent subplot engine. **Active:** bounded parallel life, up to two due rows and one new reversible line per pulse. **Sandbox:** the unbounded autonomous tier; every in-character Engine Pass creates at least four parallel events and at least two new independent durable subplots, with no narrative maximum. NPC Social or Faction Politics set to Living/Autonomous can also activate this same pipeline. Every subplot keeps its own schedule; canonical location, limited knowledge, feasible logistics, locks, player autonomy, and the persona exclusion remain hard gates. |
...
| `state_verbosity` — State Schema | **Lean** | **Lean and Full have identical extraction coverage:** both emit the complete scene/present roster and every supported durable change from every state family. **Lean** uses a compact shape, shorter notes, and less optional descriptive metadata. **Full** supplies the expanded field guide and richer supported metadata. Both receive the same Engine Pass completion budget and require a limited-knowledge private thought for every named on-stage NPC. |
...
Parallel evidence does not have to appear in visible prose. Engine Pass checks whether the character is established and alive, absent from the current scene, and able to perform the proposed activity at that location under the elapsed clock, known routes, setting geography, motives, and attached lore. A short plausibility rationale is enough when those checks pass. For example, Spike can plausibly be at the Bronze when the current canon establishes Spike, the venue, and its Sunnydale context; putting him in Beijing without a route, enough elapsed time, or lore support is rejected. An `advance` keeps the actor at their established location. A `move` needs enough time plus either depicted travel or a canon-grounded route. Facts from the visible scene still reach an absent actor only through a depicted witness, message, report, call, arrival, or consequence. Lorebooks constrain objective geography, institutions, history, objects, life-state, and physical rules; they never grant actor knowledge by themselves.
...
2. Read the warning. Evidence is semantic rather than quote-exact: an exact passage, faithful paraphrase, or relevant attached-lorebook passage is accepted when it is factually correct and materially grounds the current claim. Merely related text, contradictions, reversed actors, and partial support for a larger claim are rejected. Engine Pass derives omitted proof bookkeeping locally while still enforcing opening plot obligations, Sandbox floors, identity and time integrity, player state, life-state, feasible off-screen logistics, and knowledge transmission.
...
4. Lean and Full should capture the same content. Use Full when a model needs the expanded field guide; an omission in Lean is a compiler failure, not intended brevity.
