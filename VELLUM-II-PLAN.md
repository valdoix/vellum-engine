...
- **2026-06-26 — Phase 0 COMPLETE & shipped.** Repo `valdoix/vellum-engine` created (default branch `main`), TS+Bun+tsup+vitest. Event-log core (`events`/`reduce`/`migrate`), domain (`relations` multi-category sets, `category`, `types`), `host/user` (persisted uid) + `host/capability`, component model w/ error boundary, store (`chronicle` load/save/migrate), minimal backend + frontend shell. `bun run typecheck` clean, **13 tests pass**, `bun run build` → `dist/backend.js`+`dist/frontend.js`. Next: Phase 1 (JSON state-block parse + FOLD lifecycle + cast/relations wired live).
