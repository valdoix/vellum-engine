...
export const ParsedState = z.object({
  ...
  scene: z.object({ loc: z.string().optional().catch(undefined), time: z.string().optional().catch(undefined), clock: z.number().int().min(0).max(1439).optional().catch(undefined), tension: z.number().min(0).max(10).optional().catch(undefined), weather: z.string().optional().catch(undefined) }).optional().catch(undefined),
