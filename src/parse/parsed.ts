...
export const ParsedPresent = z.object({
  ...
  evidence: z.string().optional().catch(undefined), // exact source quote for opted-in persona fields
});
...
export const ParsedOffscreen = z.object({
  ...
  beatKind: z.enum(['progress', 'obstacle', 'consequence', 'bridge', 'resolution']).optional().catch(undefined),
  impact: z.string().optional().catch(undefined),
  grounding: z.object({
    basis: z.array(z.enum(['scene', 'character', 'location', 'lorebook', 'parallel', 'subplot', 'knowledge', 'relationship', 'intent', 'manual'])).max(10),
    rationale: z.string(),
    refs: z.array(z.string()).max(20).optional().catch(undefined),
    before: z.string().optional().catch(undefined),
    after: z.string().optional().catch(undefined),
  }).optional().catch(undefined),
  originParallel: z.object({
...
export const ParsedState = z.object({
  ...
  scene: z.object({ title: z.string().max(100).optional().catch(undefined), transition: z.enum(['continue', 'scene', 'time_skip']).optional().catch(undefined), loc: z.string().optional().catch(undefined), time: z.string().optional().catch(undefined), clock: z.number().int().min(0).max(1439).optional().catch(undefined), tension: z.number().min(0).max(10).optional().catch(undefined), weather: z.string().optional().catch(undefined) }).optional().catch(undefined),
