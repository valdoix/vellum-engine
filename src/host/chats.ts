...
export async function activeChatId(userId: string | null): Promise<string | null> {
  try {
    const c = await spindle.chats?.getActive?.(userId ?? undefined);
    return c?.id ?? null;
  } catch {
    return null;
  }
...
export async function chatNames(chatId: string, userId: string | null): Promise<{ user: string; char: string }> {
  ...
  try {
    ...
    out.char = String(compat?.character?.name || compat?.characterName || compat?.char_name || compat?.metadata?.character_name || '').trim();
