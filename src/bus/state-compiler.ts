...
Meet every active preset requirement in the supplied requirements object. For ARGENT, an empty plot ledger requires exactly one strongest grounded actionable thread and, only when no parent arc exists, exactly one clear parent arc linked by its exact title. Preserve the current parallel snapshot by omission; emit every grounded due operation and required new subplot for the active tier.
