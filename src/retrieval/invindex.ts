...
export interface RetrievableItem {
  ...
  authority?: 'canonical' | 'confirmed' | 'provisional' | 'perspective' | 'disproven';
}
...
export function collectItems(state: ChronicleState): RetrievableItem[] {
  ...
  for (const c of Object.values(state.cast)) labelsById.set(c.id, [c.name, ...(c.aka ?? []), c.role ?? ''].filter(Boolean));
  for (const f of Object.values(state.factions)) labelsById.set(f.id, [f.name, ...(f.aka ?? []), f.kind ?? ''].filter(Boolean));
  for (const l of state.locations) labelsById.set(l.id, [l.name]);
  for (const i of state.items) labelsById.set(i.id, [i.item]);
  const namesFor = (ids: readonly string[]): string => ids.flatMap((id) => labelsById.get(id) ?? [id]).filter(Boolean).join(' ');
  const entityNeedles = [...labelsById].map(([id, labels]) => ({ id, labels: labels.map((x) => x.normalize('NFKC').toLocaleLowerCase()).filter((x) => x.length >= 3) }));
  const mentionedEntities = (text: string): string[] => {
    const hay = text.normalize('NFKC').toLocaleLowerCase();
    return entityNeedles.filter((c) => c.labels.some((label) => hay.includes(label))).map((c) => c.id);
  };
  ...
  for (const k of state.knowledge) {
    ...
    put({ id: k.id, kind: 'knowledge', text, turn: k.turn, entityIds: [k.who, ...(k.about ? [k.about] : [])], knownBy: [k.who], status: `${stance}/${truth}`, audience: 'restricted', authority: 'perspective' });
  }
  for (const s of state.secrets) {
    ...
    put({ id: s.id, kind: 'secret', text, turn: s.lastTurn ?? s.formedTurn, entityIds: [...new Set([...known, ...hidden])], knownBy: known, hiddenFrom: hidden, status: hidden.length ? (s.revealed ? 'partial' : 'hidden') : 'public', audience: hidden.length ? 'restricted' : 'public', authority: 'canonical' });
  }
  ...
  const visitMemory = (m: MemorySnapshot, parents: string[] = []): void => {
    ...
    put({ id: m.id, kind, text, turn: m.turn, tier, entityIds, parentIds: parents, status: m.status, audience: 'public', authority: 'canonical' }, `${m.text} ${m.detail ?? ''} ${(m.keys ?? []).join(' ')}`);
  ...
  for (const j of state.journal) {
    ...
    put({ id: j.id, kind: 'journal', text, turn: j.turn, entityIds: entities, knownBy: [j.who], status: j.kind, audience: 'restricted', authority: 'perspective' });
  }
  for (const scar of state.scars ?? []) {
    ...
    put({ id: scar.id, kind: 'scar', text, turn: scar.turn, entityIds: entities, knownBy: [scar.who], status: 'disproven', audience: 'restricted', authority: 'disproven' });
  }
  for (const lore of state.lore ?? []) {
    ...
    put({ id: lore.id, kind: 'codex', text, turn: lore.turn, status: lore.status ?? 'confirmed', entityIds: mentionedEntities(lore.fact), audience: 'public', authority: lore.status === 'provisional' ? 'provisional' : 'confirmed' }, `${text} ${(lore.revisions ?? []).map((r) => r.fact).join(' ')}`);
  }
  for (const item of state.items ?? []) {
    ...
    put({ id: item.id, kind: 'item', text, turn: item.turn, entityIds: [item.id, ...(item.who === 'world' ? [] : [item.who])], knownBy: item.who === 'world' ? [] : [item.who], status: item.scene ? 'scene' : 'held', audience: 'public', authority: 'canonical' });
  }
  for (const [i, step] of (state.itemHistory ?? []).entries()) {
    ...
    put({ id: `item-history:${step.id}:${i}`, kind: 'item', text, turn: step.turn, entityIds: [step.itemId, ...entities], status: step.op, audience: 'public', authority: 'canonical' });
  }
  return [...byId.values()];
}
