...
export const STYLES = [
  ...
  // --- Arc spine (Chronicle > World arcs) — horizontal scrollable rail of cards.
  // Each arc shows name, beat count, and status dot (resolved/active).
  ".vle-arc-spine{display:flex;gap:10px;overflow-x:auto;padding:8px 0 10px;scrollbar-width:thin;margin-bottom:8px}",
  ".vle-arc-card{position:relative;flex:0 0 auto;min-width:220px;max-width:280px;padding:12px 13px;border:1px solid rgba(var(--vg-rgb),.2);border-left:3px solid var(--vg);background:linear-gradient(100deg,color-mix(in srgb,var(--vg) 8%,transparent),transparent 60%);border-radius:var(--vr2);transition:all .15s}",
  ".vle-arc-card:hover{border-color:rgba(var(--vg-rgb),.35);background:linear-gradient(100deg,color-mix(in srgb,var(--vg) 12%,transparent),transparent 60%)}",
  ".vle-arc-card--done{opacity:.6;border-left-color:rgba(var(--vg-rgb),.3)}",
  ".vle-arc-name{font-family:var(--vserif);font-size:calc(14.5px * var(--vscale));line-height:1.3;color:var(--vi);font-weight:600;margin-bottom:6px;overflow-wrap:anywhere}",
  ".vle-arc-meta{display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-size:calc(11px * var(--vscale));margin-bottom:6px}",
  ".vle-arc-beat-count{font:600 9px/1.4 var(--vmono);letter-spacing:.4px;text-transform:uppercase;color:var(--vle-gold);background:color-mix(in srgb,var(--vle-gold) 12%,transparent);border-radius:999px;padding:2px 7px}",
  ".vle-arc-status{font-size:8px;color:var(--v-pos)}",
  ".vle-arc-status.done{color:var(--vi2);opacity:.6}",
  ".vle-arc-status-label{font:500 9px/1.4 var(--vmono);color:var(--vi2);opacity:.7}",
  ...
  // --- Thread cards (Chronicle > World threads) — collapsible feed-style cards
  // with Establishing Shot styling. Kicker + title header always visible; toggle
  // expands to reveal latest beat, full history, meanwhile note, and actions.
  ".vle-thread-list{display:flex;flex-direction:column;gap:0;border:1px solid rgba(var(--vg-rgb),.16);border-radius:var(--vr2);overflow:hidden}",
  ".vle-thread-card{position:relative;padding:14px 16px;border-bottom:1px solid rgba(var(--vg-rgb),.12);transition:background .18s}",
  ".vle-thread-card:last-child{border-bottom:none}",
  ...
  ".vle-thread-beat{padding:10px 12px;border-left:3px solid color-mix(in srgb,var(--v-pos) 55%,transparent);background:color-mix(in srgb,var(--vi) 4%,transparent);border-radius:var(--vr1)}",
  ".vle-thread-beat-label{font:600 8.5px/1.4 var(--vmono);letter-spacing:.6px;text-transform:uppercase;color:var(--v-pos);opacity:.85;margin-bottom:5px}",
  ".vle-thread-beat-text{font-family:var(--vserif);font-size:calc(13.5px * var(--vscale));line-height:1.55;color:var(--vi)}",
  ".vle-thread-hist{border-left:3px solid rgba(var(--vg-rgb),.2);padding-left:10px}",
  ".vle-thread-hist-toggle{cursor:pointer;font:600 9px/1.4 var(--vmono);letter-spacing:.5px;text-transform:uppercase;color:var(--vi2);opacity:.7;list-style:none;user-select:none}",
  ".vle-thread-hist-toggle::-webkit-details-marker{display:none}",
  ".vle-thread-hist-toggle::before{content:'\\25B8';display:inline-block;margin-right:6px;transition:transform .15s}",
  ".vle-thread-hist[open] .vle-thread-hist-toggle::before{transform:rotate(90deg)}",
  ".vle-thread-hist-toggle:hover{color:var(--vle-gold);opacity:1}",
  ".vle-thread-hist-list{margin-top:8px;display:flex;flex-direction:column;gap:6px}",
  ".vle-thread-hist-item{font-family:var(--vserif);font-size:calc(12.5px * var(--vscale));line-height:1.5;color:var(--vi2);padding:2px 0}",
  ".vle-thread-hist-n{font:600 10px/1 var(--vmono);color:var(--vle-gold);opacity:.7;margin-right:4px}",
  ".vle-thread-meanwhile{padding:8px 11px;border-left:3px solid color-mix(in srgb,var(--v-info) 60%,transparent);background:color-mix(in srgb,var(--v-info) 8%,transparent);border-radius:var(--vr1)}",
