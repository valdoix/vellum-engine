...
const SHAPE_GEOM: Record<string, string> = {
  ...
  // The staff occupies y=9..31. Keep the card's first content row below it so
  // names, direction reads, and CRUD controls never look struck through.
  'nocturne-score': `border-radius:3px 3px calc(${R} * 3) 3px;padding:38px 15px 16px`,
  ...
  // ROCOCO — six readable porcelain objects. Ornament stays inside reserved padding;
  // the content box is never clipped or masked.
  'rococo-shell': `border-radius:calc(${R} * 2) calc(${R} * 2) 10px 10px;padding:22px 16px 14px`,
  'rococo-cartouche': `border-radius:calc(${R} * 2) 8px calc(${R} * 2) 8px;padding:16px 22px`,
  // Cast stays a practical record card: its porcelain cameo lives in the inset
  // frame instead of turning a long character row into an ellipse.
  'rococo-cameo': 'border-radius:12px 12px 7px 7px;border-width:2px;padding:20px 19px 16px',
  'rococo-ribbon': `border-radius:10px calc(${R} * 3) 10px calc(${R} * 3);border-left-width:5px;padding:18px 22px`,
  'rococo-cabinet': 'border-radius:12px 12px 5px 5px;border-width:3px;padding:18px 16px 16px',
  'rococo-medallion': 'border-radius:24px 24px 48% 48%/20px 20px 24% 24%;padding:18px 18px 21px',
  // CANDY ALCHEMIST — six pieces of laboratory glassware; no polygon cuts.
  'candy-vessel': `border-radius:10px 10px calc(${R} * 4) calc(${R} * 4);border-top-width:7px;padding:22px 16px 17px`,
  'candy-flask': `border-radius:calc(${R} * 3) calc(${R} * 3) 10px 10px;border-top-width:8px;padding:22px 18px 16px`,
  // The capsule is a boxed specimen label. The split reagent seam is decorative
  // only, leaving the cast row broad enough for names, status, and controls.
  'candy-capsule': 'border-radius:7px;border-width:2px;padding:21px 22px',
  'candy-rack': `border-radius:7px calc(${R} * 2) 7px calc(${R} * 2);border-left-width:7px;border-bottom-width:5px;padding:17px 22px`,
  'candy-drawer': 'border-radius:8px;border-width:3px;padding:18px 17px',
  'candy-lozenge': 'border-radius:999px 16px 999px 16px;padding:16px 22px',
  // DREAM QUILT — appliqué, seams and pockets with full rectangular content flow.
  'quilt-patch': `border-radius:11px calc(${R} * 3) 11px calc(${R} * 3);border-left-width:7px;padding:18px 18px 16px`,
  'quilt-join': `border-radius:calc(${R} * 3) 12px calc(${R} * 3) 12px;border-inline-width:6px;padding:18px 22px`,
  // A quilt label: square corners and stitched margins read as sewn cloth while
  // preserving the familiar wide cast-card layout.
  'quilt-locket': 'border-radius:5px;border-style:dashed;padding:23px 20px 19px',
  'quilt-runner': 'border-radius:28px 7px 28px 7px;border-block-width:5px;padding:18px 22px',
  'quilt-pocket': `border-radius:9px 9px calc(${R} * 4) calc(${R} * 4);border-top-width:7px;padding:19px 18px 21px`,
  'quilt-square': 'border-radius:13px;border-style:dashed;padding:18px',
};
...
.vlw{padding:10px 12px 28px;color:var(--vi);font:13px/1.45 var(--vserif)}.vlw>nav{display:flex;gap:5px;overflow:auto;margin-bottom:12px;padding:4px;border:1px solid var(--v-border-color);border-radius:12px;background:rgba(var(--vg-rgb),.05)}.vlw>nav button,.vlw button{border:1px solid var(--v-border-color);border-radius:8px;background:rgba(var(--vg-rgb),.06);color:var(--vi);padding:7px 10px;cursor:pointer;font:600 11px/1.2 var(--vmono);letter-spacing:.03em}.vlw>nav button.on,.vlw button.primary,.vlw button.strong{background:rgba(var(--vg-rgb),.2);border-color:var(--v-border-strong);box-shadow:var(--v-glow)}.vlw-panel{display:grid;gap:12px}.vlw-panel>header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.vlw h3,.vlw h4,.vlw p{margin:0}.vlw h3{font-size:19px}.vlw h4{font:700 10px/1.2 var(--vmono);text-transform:uppercase;letter-spacing:.12em;color:var(--vg)}.vlw p,.vlw small,.vlw-note{color:var(--vi2)}.vlw-scope{display:flex;gap:6px}.vlw-scope button.on{color:var(--vg);border-color:var(--v-border-strong)}.vlw-routes{display:grid;gap:9px}.vlw-route{display:grid;gap:8px;padding:11px;border:1px solid var(--v-border-color);border-radius:12px;background:var(--vsurf-1)}.vlw-route-head{display:flex;justify-content:space-between;gap:10px}.vlw-route-head small{display:block}.vlw-route-head>span{font:10px/1.3 var(--vmono);color:var(--v-info);text-align:right}.vlw label{display:grid;gap:4px;font:10px/1.2 var(--vmono);color:var(--vi2)}.vlw input,.vlw select{box-sizing:border-box;width:100%;min-width:0;padding:7px;border:1px solid var(--v-border-color);border-radius:7px;background:var(--vsurf-2);color:var(--vi);font:11px var(--vmono)}.vlw-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.vlw-check{display:flex!important;align-items:center;gap:7px!important}.vlw-check input{width:auto}.vlw-panel>footer{display:flex;justify-content:flex-end;gap:7px;position:sticky;bottom:0;padding:9px;background:var(--vle-bg-solid);border-top:1px solid var(--v-border-color)}.vlw-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.vlw-action{text-align:left!important;display:grid;gap:4px;white-space:normal}.vlw-action span{font-weight:400;color:var(--vi2);letter-spacing:0}.vlw-callout,.vlw-progress,.vlw-candidate{padding:11px;border:1px solid var(--v-border-color);border-left:3px solid var(--vg);border-radius:9px;background:rgba(var(--vg-rgb),.07)}.vlw-progress{display:grid;gap:5px}.vlw-progress progress{width:100%;accent-color:var(--vg)}.vlw-score{display:flex;align-items:baseline;gap:8px}.vlw-score strong{font-size:38px;color:var(--vg)}.vlw-counts{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px}.vlw-counts span{display:grid;padding:7px;border:1px solid var(--v-border-color);border-radius:8px;color:var(--vi2);font:9px var(--vmono)}.vlw-counts b{font-size:16px;color:var(--vi)}.vlw-findings{display:grid;gap:7px;padding:0;margin:0;list-style:none}.vlw-findings li{display:grid;padding:9px;border:1px solid var(--v-border-color);border-left:3px solid var(--v-info);border-radius:8px}.vlw-findings li.warning{border-left-color:var(--v-press)}.vlw-findings li.error{border-left-color:var(--v-neg)}.vlw-findings li.ok{border-left-color:var(--v-pos)}.vlw-findings span{color:var(--vi2);font:10px var(--vmono)}
@media(max-width:520px){.vlw-grid,.vlw-actions{grid-template-columns:1fr}.vlw-counts{grid-template-columns:repeat(2,minmax(0,1fr))}.vlw-panel>header{align-items:stretch;flex-direction:column}}

/* Workbench: one quiet control room shared by routing, interventions, recovery,
   and health. Uses the active VELLUM skin variables instead of a fixed palette. */
.vlw{padding:0 12px 30px;color:var(--vi);font:13px/1.48 var(--vserif)}
.vlw>nav{display:flex;align-items:center;gap:3px;overflow-x:auto;margin:0 -2px 16px;padding:0 4px;border:0;border-bottom:1px solid var(--v-border-color);border-radius:0;background:transparent;scrollbar-width:thin}
.vlw>nav button{position:relative;display:inline-flex;align-items:center;justify-content:center;gap:7px;min-height:46px;padding:0 13px;border:0;border-radius:0;background:transparent;box-shadow:none;color:var(--vi2);opacity:.76;text-transform:uppercase;white-space:nowrap}
.vlw>nav button i{font:13px/1 var(--vserif);font-style:normal}.vlw>nav button em{display:grid;place-items:center;min-width:17px;height:17px;padding:0 4px;border-radius:99px;background:rgba(var(--vg-rgb),.16);color:var(--vg);font:700 9px var(--vmono);font-style:normal}
.vlw>nav button.on{border:0;background:linear-gradient(180deg,transparent,rgba(var(--vg-rgb),.1));box-shadow:none;color:var(--vg);opacity:1}.vlw>nav button.on:after{content:"";position:absolute;right:25%;bottom:0;left:25%;height:2px;border-radius:4px;background:var(--vg);box-shadow:var(--v-glow)}
.vlw>main{min-width:0}.vlw-panel{display:grid;gap:13px}.vlw-panel-head{display:flex;align-items:flex-start;justify-content:space-between;gap:15px;margin:0}.vlw-panel-head>div{min-width:0}.vlw-panel h2{margin:2px 0 3px;font-size:23px;font-weight:500;line-height:1.15}.vlw-panel h3{margin:2px 0 3px;font-size:16px;font-weight:600;line-height:1.25}.vlw-panel p{margin:0;color:var(--vi2)}
.vlw-label{display:block;color:var(--vg);font:700 9px/1.3 var(--vmono);letter-spacing:.11em;text-transform:uppercase}.vlw-card{min-width:0;padding:13px;border:1px solid var(--v-border-color);border-radius:11px;background:linear-gradient(145deg,var(--vsurf-1),var(--vsurf-2));box-shadow:inset 0 1px rgba(255,255,255,.035),var(--v-shadow-card)}
.vlw-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.vlw-pill{display:inline-flex;align-items:center;gap:6px;flex:none;padding:4px 7px;border-radius:99px;background:rgba(var(--vg-rgb),.09);color:var(--vi2);font:700 8px/1.2 var(--vmono);letter-spacing:.05em;text-transform:uppercase;white-space:nowrap}.vlw-pill i{width:6px;height:6px;border-radius:50%;background:currentColor;box-shadow:0 0 8px currentColor}.vlw-pill.good{color:var(--v-pos)}.vlw-pill.warn{color:var(--v-press)}
.vlw button{border:1px solid var(--v-border-color);border-radius:8px;background:rgba(var(--vg-rgb),.055);color:var(--vi);padding:7px 10px;cursor:pointer;font:650 10px/1.2 var(--vmono);letter-spacing:.025em}.vlw button:hover{border-color:var(--v-border-strong);background:rgba(var(--vg-rgb),.11)}.vlw button.primary,.vlw button.strong{border-color:var(--v-border-strong);background:rgba(var(--vg-rgb),.2);box-shadow:var(--v-glow);color:var(--vi)}.vlw button:focus-visible,.vlw select:focus-visible,.vlw input:focus-visible{outline:2px solid var(--vg);outline-offset:2px}
.vlw-scope{display:flex;align-items:center;gap:5px;width:max-content;padding:3px;border:1px solid var(--v-border-color);border-radius:10px;background:rgba(var(--vg-rgb),.035)}.vlw-scope button{border:0;background:transparent;box-shadow:none}.vlw-scope button.on{background:rgba(var(--vg-rgb),.13);box-shadow:inset 0 0 0 1px var(--v-border-color);color:var(--vg)}
.vlw-model-layout{display:grid;grid-template-columns:minmax(155px,31%) minmax(0,1fr);gap:12px}.vlw-role-rail{min-width:0;padding:5px 11px 5px 0;border-right:1px solid var(--v-border-color)}.vlw-role-rail>p{margin:3px 0 7px;font:9px var(--vmono)}.vlw-role{display:flex!important;align-items:center;justify-content:space-between;gap:7px;width:100%;padding:8px 7px!important;border:0!important;border-left:2px solid transparent!important;border-radius:0 7px 7px 0!important;background:transparent!important;box-shadow:none!important;text-align:left}.vlw-role span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.vlw-role small{max-width:44%;overflow:hidden;color:var(--vi2);font:8px var(--vmono);text-overflow:ellipsis;white-space:nowrap}.vlw-role.on{border-left-color:var(--vg)!important;background:rgba(var(--vg-rgb),.105)!important;color:var(--vi)}.vlw-role.on small{color:var(--vg)}
.vlw-model-cards{display:grid;grid-template-columns:minmax(0,1.12fr) minmax(170px,.88fr);gap:9px;align-items:start}.vlw-route-editor{grid-row:span 2;display:grid;gap:12px}.vlw-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.vlw-fields label,.vlw-fallback label{display:grid;gap:4px;color:var(--vi2);font:700 9px/1.3 var(--vmono);letter-spacing:.04em;text-transform:uppercase}.vlw-fields label.wide{grid-column:1/-1}.vlw input,.vlw select{box-sizing:border-box;width:100%;min-width:0;padding:8px;border:1px solid var(--v-border-color);border-radius:7px;background:var(--vsurf-2);color:var(--vi);font:11px var(--vmono);letter-spacing:0;text-transform:none}.vlw-check{grid-column:1/-1;display:flex!important;align-items:center;gap:8px!important;padding:7px;border:1px solid var(--v-border-color);border-radius:7px;background:rgba(var(--vg-rgb),.035);text-transform:none!important}.vlw-check input{width:auto}.vlw-check span{display:grid}.vlw-check b{color:var(--vi);font-size:10px}.vlw-check small{color:var(--vi2);font-weight:400}
.vlw-effective small,.vlw-empty-inline{display:block;color:var(--vi2);font:9px/1.4 var(--vmono)}.vlw-meter{height:5px;overflow:hidden;margin:11px 0 7px;border-radius:99px;background:rgba(var(--vg-rgb),.08)}.vlw-meter i{display:block;width:76%;height:100%;background:linear-gradient(90deg,var(--v-info),var(--vg))}.vlw-fallback{display:grid;gap:10px;margin-top:10px}.vlw-route-path{display:flex;align-items:center;gap:5px;flex-wrap:wrap}.vlw-route-node{display:inline-flex;align-items:center;gap:4px;padding:4px 4px 4px 7px;border:1px solid var(--v-border-color);border-radius:7px;background:var(--vsurf-2);font:9px var(--vmono)}.vlw-route-node button{padding:1px 4px;border:0;background:transparent;color:var(--vi2)}.vlw-panel>footer{position:sticky;bottom:0;z-index:2;display:flex;justify-content:flex-end;gap:7px;padding:9px 0;background:linear-gradient(180deg,transparent,var(--vle-bg-solid) 25%);border:0}
.vlw-hero{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:12px;padding:14px;border:1px solid var(--v-border-strong);border-radius:12px;background:linear-gradient(100deg,rgba(var(--vg-rgb),.16),rgba(var(--vg-rgb),.035))}.vlw-hero-seal{display:grid;place-items:center;width:40px;height:40px;border:1px solid var(--v-border-strong);border-radius:50%;color:var(--vg);font-size:18px;box-shadow:var(--v-glow)}.vlw-action-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.vlw-action-card{position:relative;display:flex;min-height:139px;overflow:hidden;flex-direction:column;justify-content:space-between}.vlw-action-card:before{content:"";position:absolute;top:0;bottom:0;left:0;width:3px;background:var(--v-info)}.vlw-action-card.ready:before{background:var(--v-pos)}.vlw-action-card.attention:before{background:var(--v-press)}.vlw-action-foot{display:flex;align-items:center;justify-content:space-between;gap:7px;margin-top:13px;color:var(--vi2);font:9px var(--vmono)}
.vlw-callout{padding:10px 12px;border-left:3px solid var(--v-info);border-radius:8px;background:color-mix(in srgb,var(--v-info) 8%,transparent);color:var(--vi2)}.vlw-progress{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:7px 12px;padding:11px;border:1px solid var(--v-border-color);border-radius:9px;background:rgba(var(--vg-rgb),.055)}.vlw-progress>div{display:grid}.vlw-progress span,.vlw-progress small{color:var(--vi2)}.vlw-progress progress{grid-column:1/-1;width:100%;accent-color:var(--vg)}.vlw-recovery-grid{display:grid;grid-template-columns:minmax(0,1.2fr) minmax(190px,.8fr);gap:10px;align-items:start}.vlw-recovery-grid>aside,.vlw-health-grid>aside{display:grid;gap:9px}.vlw-candidate{border-color:color-mix(in srgb,var(--v-info) 42%,var(--v-border-color));background:linear-gradient(145deg,color-mix(in srgb,var(--v-info) 7%,var(--vsurf-1)),var(--vsurf-2))}.vlw-candidate-empty{display:grid;align-content:center;justify-items:start;min-height:240px}.vlw-candidate-empty button{margin-top:14px}.vlw-counts{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px;margin-top:13px}.vlw-counts>span,.vlw-coverage>span{display:grid;padding:7px;border:1px solid var(--v-border-color);border-radius:7px;background:rgba(var(--vg-rgb),.035)}.vlw-counts b{font-size:13px}.vlw-counts b i{color:var(--v-info);font-style:normal}.vlw-counts small,.vlw-coverage small{color:var(--vi2);font:8px var(--vmono);text-transform:uppercase}.vlw-findings{display:grid;margin-top:10px}.vlw-finding{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 0;border-top:1px solid var(--v-border-color)}.vlw-finding span{display:grid}.vlw-finding small{color:var(--vi2)}.vlw-finding em{padding:3px 6px;border-radius:99px;background:rgba(var(--vg-rgb),.08);color:var(--v-press);font:8px var(--vmono);font-style:normal;text-transform:uppercase}.vlw-finding.ok em{color:var(--v-pos)}.vlw-candidate-actions{display:flex;justify-content:flex-end;gap:7px;margin-top:10px}.vlw-rebuild-log{display:grid;margin:7px 0}.vlw-log-row{display:grid;grid-template-columns:25px 1fr;gap:7px;padding:8px 0;border-top:1px solid var(--v-border-color)}.vlw-log-row:first-child{border-top:0}.vlw-log-row b{color:var(--vi2);font:9px var(--vmono)}.vlw-log-row.done b{color:var(--v-info)}.vlw-log-row span{color:var(--vi2)}.vlw-backup{border-color:color-mix(in srgb,var(--v-info) 28%,var(--v-border-color))}.vlw-backup button{margin-top:10px}
.vlw-health-summary{display:grid;grid-template-columns:150px minmax(0,1fr);gap:9px}.vlw-health-score{display:grid;place-items:center;text-align:center;border-color:color-mix(in srgb,var(--v-info) 40%,var(--v-border-color));background:radial-gradient(circle,rgba(var(--vg-rgb),.08),transparent 68%),var(--vsurf-1)}.vlw-score-ring{position:relative;display:grid;place-items:center;width:98px;height:98px;margin-bottom:5px;border-radius:50%;background:conic-gradient(var(--v-info) 0 var(--vlw-score),rgba(var(--vg-rgb),.11) var(--vlw-score) 360deg)}.vlw-score-ring:after{content:"";position:absolute;inset:7px;border:1px solid var(--v-border-color);border-radius:50%;background:var(--vsurf-1)}.vlw-score-ring strong,.vlw-score-ring small{position:relative;z-index:1}.vlw-score-ring strong{font-size:32px;font-weight:500;color:var(--v-info)}.vlw-score-ring small{position:absolute;margin-top:42px;color:var(--vi2);font:8px var(--vmono);text-transform:uppercase}.vlw-health-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.vlw-health-stats>.vlw-card{display:flex;min-height:110px;flex-direction:column;justify-content:center}.vlw-health-stats b{font-size:23px;font-weight:500}.vlw-health-grid{display:grid;grid-template-columns:minmax(0,1.2fr) minmax(190px,.8fr);gap:9px;align-items:start}.vlw-check-list{display:grid;margin-top:7px}.vlw-check-row{display:grid;grid-template-columns:22px minmax(0,1fr) auto;align-items:center;gap:8px;padding:9px 0;border-top:1px solid var(--v-border-color)}.vlw-check-row:first-child{border-top:0}.vlw-check-row>i{display:grid;place-items:center;width:20px;height:20px;border-radius:50%;background:color-mix(in srgb,var(--v-info) 14%,transparent);color:var(--v-info);font:700 9px var(--vmono);font-style:normal}.vlw-check-row.warning>i{color:var(--v-press)}.vlw-check-row.error>i{color:var(--v-neg)}.vlw-check-row.ok>i{color:var(--v-pos)}.vlw-check-row span{display:grid}.vlw-check-row small{color:var(--vi2)}.vlw-check-row em{color:var(--vi2);font:8px var(--vmono);font-style:normal;text-transform:uppercase}.vlw-next{border-color:color-mix(in srgb,var(--v-press) 38%,var(--v-border-color))}.vlw-next button{width:100%;margin-top:11px}.vlw-coverage{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px;margin-top:9px}.vlw-coverage b{font-size:13px}
@media(max-width:700px){.vlw-model-layout,.vlw-model-cards,.vlw-recovery-grid,.vlw-health-summary,.vlw-health-grid{grid-template-columns:1fr}.vlw-role-rail{display:flex;gap:4px;overflow-x:auto;padding:0 0 10px;border-right:0;border-bottom:1px solid var(--v-border-color)}.vlw-role-rail>.vlw-label,.vlw-role-rail>p{display:none}.vlw-role{flex:0 0 auto!important;width:auto!important;border-left:0!important;border-bottom:2px solid transparent!important;border-radius:7px!important}.vlw-role.on{border-left:0!important;border-bottom-color:var(--vg)!important}.vlw-role small{display:none}.vlw-route-editor{grid-row:auto}.vlw-action-grid{grid-template-columns:1fr}.vlw-health-stats{grid-template-columns:repeat(3,minmax(0,1fr))}}
@media(max-width:520px){.vlw{padding-right:8px;padding-left:8px}.vlw-panel-head,.vlw-card-head{align-items:stretch;flex-direction:column}.vlw-hero{grid-template-columns:auto minmax(0,1fr)}.vlw-hero>button{grid-column:1/-1;width:100%}.vlw-fields{grid-template-columns:1fr}.vlw-fields label.wide,.vlw-check{grid-column:auto}.vlw-counts{grid-template-columns:repeat(2,minmax(0,1fr))}.vlw-health-stats{grid-template-columns:1fr}.vlw-health-stats>.vlw-card{min-height:80px}.vlw-panel>footer{flex-direction:column}.vlw-panel>footer button{width:100%}}
`,
  ...
  ".vle-head{display:flex;align-items:center;flex-wrap:wrap;gap:9px;font-size:calc(22px * var(--vscale));letter-spacing:1.5px;padding-bottom:calc(11px * var(--vscale));border-bottom:1px solid var(--vle-gold-soft)}",
  ".vle-brand{display:inline-flex;align-items:baseline;gap:9px;flex:0 0 auto;min-width:max-content;white-space:nowrap}",
  ".vle-mark{display:inline-flex;align-items:baseline;flex:none;white-space:nowrap}",
  ...
  ".vle-stats{margin-left:auto;min-width:0;display:flex;align-items:center;justify-content:flex-end;flex:1 1 260px;flex-wrap:wrap;gap:calc(5px * var(--vscale))}",
  ...
  // --- Atlas: locations as gazetteer plates (region spine + breadcrumb + recency pulse) ---
  ".vle-atlas{margin:0 0 6px;display:flex;flex-direction:column;gap:7px}",
  ".vle-atlas-plate{position:relative;display:flex;gap:0;border:1px solid rgba(var(--vg-rgb),.16);border-radius:var(--vr1);background:var(--vsurf-1);overflow:hidden;transition:border-color .12s,background .12s}",
  ".vle-atlas-plate:hover{border-color:rgba(var(--vg-rgb),.32);background:rgba(var(--vg-rgb),.04)}",
  // region-hued left spine: hue set inline per root ancestor
  ".vle-atlas-spine{flex:0 0 auto;width:4px;align-self:stretch;background:hsl(var(--atlas-hue,45),42%,52%)}",
  ".vle-atlas-plate.here{border-color:rgba(var(--vg-rgb),.5);background:color-mix(in srgb,var(--vle-gold) 8%,transparent);box-shadow:0 0 0 1px rgba(var(--vg-rgb),.28),0 0 18px -6px rgba(var(--vg-rgb),.5)}",
  ".vle-atlas-plate.here .vle-atlas-spine{background:var(--vle-gold)}",
  ".vle-atlas-body{flex:1 1 auto;min-width:0;padding:7px 9px}",
  ".vle-atlas-top{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap}",
  ".vle-atlas-name{font-family:var(--vserif);font-size:calc(15px * var(--vscale));font-weight:600;color:var(--vi)}",
  ".vle-atlas-here{font:600 8.5px/1 var(--vmono);letter-spacing:.7px;text-transform:uppercase;color:var(--vle-gold);border:1px solid rgba(var(--vg-rgb),.4);border-radius:999px;padding:2px 7px}",
  ".vle-atlas-kind{margin-left:auto;flex:0 0 auto;font:600 9px/1 var(--vmono);letter-spacing:.4px;border-radius:999px;padding:2px 7px;white-space:nowrap}",
  ".vle-atlas-kind.pin{color:var(--vle-gold);background:rgba(var(--vg-rgb),.14)}",
  ".vle-atlas-kind.auto{color:var(--vi2);background:rgba(var(--vi-rgb,180,170,150),.1);opacity:.7}",
  ".vle-atlas-crumb{margin:3px 0 0;font:500 10px/1.4 var(--vmono);color:var(--vi2);opacity:.62}",
  ".vle-atlas-crumb .sep{opacity:.5;margin:0 1px}",
  ".vle-atlas-crumb .cur{color:var(--vle-gold);opacity:.9}",
  ".vle-atlas-note{margin:4px 0 0;font-size:calc(12.5px * var(--vscale));color:var(--vi2);opacity:.8;line-height:1.4}",
  ".vle-atlas-foot{display:flex;align-items:center;gap:10px;margin:6px 0 0;flex-wrap:wrap}",
  ".vle-atlas-seen{display:inline-flex;align-items:center;gap:5px;font:500 10px/1 var(--vmono);color:var(--vi2);opacity:.7}",
  ".vle-atlas-seen .dot{width:6px;height:6px;border-radius:50%;background:var(--vle-gold)}",
  ".vle-atlas-seen.fresh .dot{background:var(--vle-gold);box-shadow:0 0 6px 1px rgba(var(--vg-rgb),.6)}",
  ".vle-atlas-seen.warm .dot{background:hsl(45,50%,55%);opacity:.85}",
  ".vle-atlas-seen.cool .dot{background:hsl(45,25%,50%);opacity:.6}",
  ".vle-atlas-seen.cold .dot{background:var(--vi2);opacity:.35}",
  ".vle-atlas-childtag{font:500 9.5px/1 var(--vmono);color:var(--vi2);opacity:.55}",
  ".vle-atlas-childtag.toggle{display:inline-flex;align-items:center;gap:3px;background:transparent;border:1px solid rgba(var(--vg-rgb),.2);border-radius:999px;padding:2px 7px 2px 5px;cursor:pointer;opacity:.7;transition:opacity .12s,background .12s,border-color .12s}",
  ".vle-atlas-childtag.toggle:hover{opacity:1;background:rgba(var(--vg-rgb),.1);border-color:rgba(var(--vg-rgb),.4);color:var(--vle-gold)}",
  ".vle-atlas-caret{display:inline-block;font-size:8px;line-height:1;color:var(--vle-gold);transition:transform .12s}",
  ".vle-atlas-childtag.closed .vle-atlas-caret{transform:rotate(-90deg)}",
  ".vle-atlas-ctl{margin-left:auto;display:inline-flex;gap:4px;opacity:0;transition:opacity .12s}",
  ".vle-atlas-plate:hover .vle-atlas-ctl{opacity:1}",
  ...
  "html[data-vle-chrome='futuristic'] .vle-dir-col-h{font-family:var(--vmono);color:var(--vg)}",
  "html[data-vle-chrome='futuristic'] .vle-dir-card{border-radius:2px;background:rgba(var(--vg-rgb),.06)}",
  "html[data-vle-chrome='futuristic'] .vle-dir-text,html[data-vle-chrome='futuristic'] .vle-atlas-name,html[data-vle-chrome='futuristic'] .vle-ns-v{font-family:var(--vmono);font-style:normal}",
  "html[data-vle-chrome='futuristic'] .vle-atlas-plate{border-radius:2px}",
  "html[data-vle-chrome='modern'] .vle-dir-col-h{color:var(--vi2);opacity:.6}",
  ...
  ".vle-act-item{display:flex;align-items:center;justify-content:space-between;gap:10px;min-width:0;font:600 calc(11px * var(--vscale))/1 var(--vmono);letter-spacing:.4px;text-transform:uppercase;color:var(--vi2);background:rgba(var(--vg-rgb),.06);border:1px solid var(--vle-gold-soft);border-radius:8px;padding:9px 12px;cursor:pointer;transition:background .15s,color .15s;text-align:left;width:100%;overflow:hidden}",
  ".vle-act-item .vi{opacity:.8;margin-right:-2px}",
  ".vle-act-item .vle-act-l{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
  ...
  ".vle-act-st{flex:none;max-width:34%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font:600 calc(9px * var(--vscale))/1 var(--vmono);letter-spacing:.3px;text-transform:none;color:var(--vg);opacity:.85}",
  ...
  ".vle-suggest-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin:8px 0 16px}",
  ".vle-suggest-card{position:relative;overflow:hidden;padding:14px 15px;border:1px solid color-mix(in srgb,var(--vi) 28%,transparent);border-radius:var(--vr2);background:linear-gradient(145deg,color-mix(in srgb,var(--vi) 10%,var(--vsurf-1)),color-mix(in srgb,var(--vsurf-1) 96%,transparent));box-shadow:inset 3px 0 0 color-mix(in srgb,var(--vi) 65%,transparent)}",
  ".vle-suggest-kicker{font:700 9px/1.4 var(--vmono);letter-spacing:.16em;text-transform:uppercase;color:var(--vi)}",
  ".vle-suggest-title{font:700 16px/1.25 var(--vserif);margin:7px 0 5px;color:var(--vi)}",
  ".vle-suggest-note{font-size:12px;line-height:1.45;color:var(--vi);opacity:.86}",
  ".vle-suggest-reason{font-size:10px;line-height:1.4;opacity:.58;margin-top:9px;padding-top:8px;border-top:1px solid rgba(var(--vg-rgb),.12)}",
  ".vle-suggest-actions{display:flex;gap:7px;margin-top:11px}",
  ".vle-suggest-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin:8px 0 16px}",
  ".vle-suggest-card{position:relative;overflow:hidden;padding:14px 15px;border:1px solid color-mix(in srgb,var(--vi) 28%,transparent);border-radius:calc(var(--v-radius) * .72);background:linear-gradient(145deg,color-mix(in srgb,var(--vi) 10%,var(--v-card)),color-mix(in srgb,var(--v-card) 96%,transparent));box-shadow:inset 3px 0 0 color-mix(in srgb,var(--vi) 65%,transparent)}",
  ".vle-suggest-kicker{font:700 9px/1.4 var(--vmono);letter-spacing:.16em;text-transform:uppercase;color:var(--vi)}",
  ".vle-suggest-title{font:700 16px/1.25 var(--vserif);margin:7px 0 5px;color:var(--vi)}",
  ".vle-suggest-note{font-size:12px;line-height:1.45;color:var(--vi);opacity:.86}",
  ".vle-suggest-reason{font-size:10px;line-height:1.4;opacity:.58;margin-top:9px;padding-top:8px;border-top:1px solid rgba(var(--vg-rgb),.12)}",
  ...
  ".vle-bc-head{display:flex;align-items:center;gap:8px;flex-wrap:wrap;min-width:0}",
  ".vle-bc-pair{min-width:0;font-family:var(--vserif);font-size:calc(15px * var(--vscale));font-weight:600;letter-spacing:.2px;overflow-wrap:anywhere}",
  ".vle-bc-reads{display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap;min-width:0}",
  ...
  ".vle-bc .vle-rel-ctl{margin-left:auto;flex-wrap:wrap;justify-content:flex-end}",
  ...
  ".vle-bc-dot--edge-l .vle-bc-val{left:6px;right:auto;transform:none}",
  ".vle-bc-dot--edge-r .vle-bc-val{left:auto;right:6px;transform:none}",
  ...
  ".vlf-title{min-width:0;overflow:hidden;white-space:nowrap;font-family:var(--vserif);font-size:calc(20px * var(--vscale));letter-spacing:3px;text-transform:uppercase;color:var(--vi)}",
  ".vlf-actions{margin-left:auto;min-width:0;display:flex;gap:3px}",
  ".vlf-act{flex:0 1 auto;min-width:0;white-space:nowrap;font:600 calc(10px * var(--vscale))/1 var(--vmono);letter-spacing:.5px;text-transform:uppercase;color:var(--vi2);background:rgba(var(--vg-rgb),.1);border:1px solid rgba(var(--vg-rgb),.26);border-radius:8px;padding:6px 7px;cursor:pointer;transition:background .15s}",
  ...
  ".vlf-engine-held{display:flex;align-items:center;gap:10px;margin:8px 10px 0;padding:9px 10px;border:1px solid color-mix(in srgb,var(--v-warn,#d6a64b) 48%,transparent);border-radius:9px;background:color-mix(in srgb,var(--v-warn,#d6a64b) 10%,var(--vsurf-1));font:600 10px/1.25 var(--vmono);color:var(--vi)}",
  ".vlf-engine-held>span{min-width:0;display:flex;flex:1;flex-direction:column;gap:3px}.vlf-engine-held b{color:var(--v-warn,#d6a64b);letter-spacing:.4px}.vlf-engine-held small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font:500 9px/1.3 var(--vmono);color:var(--vi2)}",
  ".vlf-engine-held button{flex:0 0 auto;border:1px solid color-mix(in srgb,var(--v-warn,#d6a64b) 60%,transparent);border-radius:7px;padding:6px 8px;background:color-mix(in srgb,var(--v-warn,#d6a64b) 14%,transparent);color:var(--vi);font:700 9px/1 var(--vmono);text-transform:uppercase;cursor:pointer}.vlf-engine-held button:hover{background:color-mix(in srgb,var(--v-warn,#d6a64b) 24%,transparent)}.vlf-engine-held button:disabled{cursor:wait;opacity:.6}",
  ...
  // ---- modal ----
  ".vlfm-overlay{position:fixed;inset:0;z-index:10000;display:grid;place-items:center;padding:12px;box-sizing:border-box;background:rgba(8,7,5,.6);backdrop-filter:blur(3px)}",
  ...
  // ---- live summarizer process window ----
  ".vle-sumwin{position:fixed;right:18px;bottom:18px;z-index:10001;box-sizing:border-box;width:min(560px,calc(100vw - 28px));max-height:min(78vh,760px);display:flex;flex-direction:column;overflow:hidden;color:var(--vle-ink,#e5dcc8);background:linear-gradient(165deg,rgba(29,25,18,.99),rgba(13,12,9,.99));border:1px solid rgba(var(--vg-rgb,205,168,78),.55);border-radius:14px;box-shadow:0 24px 75px rgba(0,0,0,.72),0 0 24px rgba(var(--vg-rgb,205,168,78),.11)}",
  ".vle-sumwin-head{display:flex;align-items:center;gap:8px;min-height:42px;padding:0 10px 0 14px;border-bottom:1px solid rgba(var(--vg-rgb,205,168,78),.22);background:rgba(var(--vg-rgb,205,168,78),.055);cursor:grab;touch-action:none;user-select:none}.vle-sumwin.is-dragging .vle-sumwin-head{cursor:grabbing}.vle-sumwin.is-dragging{transition:none}",
  ".vle-sumwin-pulse{width:8px;height:8px;flex:none;border-radius:50%;background:var(--vle-gold,#cda84e);box-shadow:0 0 0 0 rgba(var(--vg-rgb,205,168,78),.5);animation:vle-sum-pulse 1.6s ease-out infinite}",
  ".vle-sumwin.is-done .vle-sumwin-pulse{background:var(--v-pos-i,#83b99a);animation:none}.vle-sumwin.is-failed .vle-sumwin-pulse{background:var(--v-warn,#b795d6);animation:none}",
  "@keyframes vle-sum-pulse{70%{box-shadow:0 0 0 7px rgba(var(--vg-rgb,205,168,78),0)}100%{box-shadow:0 0 0 0 rgba(var(--vg-rgb,205,168,78),0)}}",
  "html[data-vle-motion='off'] .vle-sumwin-pulse{animation:none}",
  ".vle-sumwin-title{font:600 14px/1 var(--vserif);letter-spacing:1.1px;text-transform:uppercase;color:var(--vi,#eee4d0)}",
  ".vle-sumwin-mode{margin-right:auto;padding:3px 7px;border:1px solid rgba(var(--vg-rgb,205,168,78),.25);border-radius:999px;font:600 8px/1 var(--vmono);letter-spacing:.6px;text-transform:uppercase;color:var(--vle-gold,#cda84e)}",
  ".vle-sumwin-icon{width:26px;height:26px;display:grid;place-items:center;padding:0;border:0;border-radius:6px;background:transparent;color:var(--vi2,#d2c7b0);font:500 18px/1 sans-serif;cursor:pointer}.vle-sumwin-icon:hover{background:rgba(var(--vg-rgb,205,168,78),.15);color:var(--vle-gold,#cda84e)}",
  ".vle-sumwin-body{min-height:0;padding:12px 14px;overflow-y:auto;display:flex;flex-direction:column;gap:9px}.vle-sumwin.is-min .vle-sumwin-body,.vle-sumwin.is-min .vle-sumwin-foot{display:none}",
  ".vle-sumwin-status{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;font:600 10px/1.45 var(--vmono);color:var(--vi2,#d2c7b0)}.vle-sumwin-status [data-sum-tokens]{flex:none;color:var(--vle-gold,#cda84e);opacity:.85}",
  ".vle-sumwin-track{height:3px;overflow:hidden;border-radius:3px;background:rgba(var(--vg-rgb,205,168,78),.13)}.vle-sumwin-track span{display:block;width:2%;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--vle-gold,#cda84e),var(--vi,#eee4d0));transition:width .25s ease}",
  ".vle-sumwin-meta{font:500 9px/1.4 var(--vmono);letter-spacing:.25px;color:var(--vle-dim,#948b78)}",
  ".vle-sumwin-pass{overflow:hidden;border:1px solid rgba(var(--vg-rgb,205,168,78),.18);border-radius:9px;background:rgba(7,7,6,.38)}",
  ".vle-sumwin-passhead{display:flex;justify-content:space-between;gap:8px;padding:7px 9px;border-bottom:1px solid rgba(var(--vg-rgb,205,168,78),.13);font:600 8.5px/1 var(--vmono);letter-spacing:.65px;text-transform:uppercase;color:var(--vle-gold,#cda84e)}.vle-sumwin-passhead span:last-child{opacity:.55}",
  ...
  ".vlfm-head{display:flex;flex:0 0 auto;align-items:center;gap:8px;padding:13px 16px;font-family:var(--vserif);font-size:18px;letter-spacing:1.5px;text-transform:uppercase;color:var(--vi);border-bottom:1px solid rgba(var(--vg-rgb),.22)}",
  ".vlfm-mark{color:var(--vle-gold)}",
  ".vlfm-body{min-height:0;padding:14px 16px;overflow-y:auto;display:flex;flex-direction:column;gap:11px}",
  ...
  ".vlfm-foot{display:flex;flex:0 0 auto;align-items:center;justify-content:flex-end;gap:8px;padding:11px 16px;border-top:1px solid rgba(var(--vg-rgb),.18)}",
  ".vlfm-foot-actions{display:flex;flex:1 1 auto;min-width:0;gap:8px;flex-wrap:wrap}",
  ".vlfm-foot-primary{display:flex;flex:0 0 auto;gap:8px;margin-left:auto}",
  "@container (max-width:520px){.vlfm-foot{align-items:stretch;flex-wrap:wrap}.vlfm-foot-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));flex-basis:100%}.vlfm-foot-actions .vlfm-btn{width:100%;min-width:0;padding-inline:9px;white-space:normal;line-height:1.15}.vlfm-foot-actions .vlfm-btn:only-child{grid-column:1/-1}.vlfm-foot-primary{width:100%;justify-content:flex-end}}",
  ...
  ".vlv-bk-tag{font:600 9px/1 var(--vmono);text-transform:uppercase;color:var(--v-info);opacity:.7}.vlv-bk-tag.vault{color:var(--v-pos-i);opacity:1}.vlv-bk-tag.shared{color:var(--v-info);opacity:1}",
  ".vlv-bk-ctl{display:inline-flex;align-items:center;gap:5px}",
  ".vlv-bk-att,.vlv-bk-claim{font:600 9px/1 var(--vmono);letter-spacing:.4px;color:var(--vi2);background:rgba(var(--vg-rgb),.08);border:1px solid rgba(var(--vg-rgb),.22);border-radius:6px;padding:4px 8px;cursor:pointer;white-space:nowrap}.vlv-bk-claim{color:var(--vle-gold);border-color:rgba(var(--vg-rgb),.4);background:rgba(var(--vg-rgb),.14)}.vlv-bk-claim:hover{background:rgba(var(--vg-rgb),.27)}",
  ...
  ".vlv-adopt{display:flex;align-items:center;gap:12px;padding:9px 10px;border:1px solid rgba(var(--vg-rgb),.35);border-radius:9px;background:rgba(var(--vg-rgb),.08)}.vlv-adopt>div{min-width:0;display:flex;flex-direction:column;gap:3px}.vlv-adopt b{font:600 11px/1.25 var(--vmono);color:var(--vle-gold)}.vlv-adopt span{font-size:11px;line-height:1.35;color:var(--vi2)}.vlv-adopt button{margin-left:auto;flex:none}",
  ...
  ".vle-mode-sk.sk-rococo{flex-direction:column;gap:4px;background:radial-gradient(circle at 78% 22%,#acefc3 0 3px,transparent 4px),linear-gradient(150deg,#fff3f8,#effaf2)}.vle-mode-sk.sk-rococo i{height:6px;border:1px double #c9548d;border-radius:14px 4px 14px 4px;background:linear-gradient(90deg,#ffacd4,#acefc3)}.vle-mode-sk.sk-rococo i:first-child{height:10px;border-radius:20px 20px 5px 5px}.vle-mode-sk.sk-rococo i:last-child{width:62%;margin-inline:auto;background:linear-gradient(90deg,#b8dcff,#d7b9ff)}",
  ".vle-mode-sk.sk-candy{flex-direction:column;gap:4px;background:radial-gradient(circle at 10px 8px,#40ff87 0 1px,transparent 2px) 0 0/15px 15px,linear-gradient(150deg,#160d24,#07040e)}.vle-mode-sk.sk-candy i{height:6px;border:1px solid #20d9ff;border-radius:999px;background:linear-gradient(90deg,#ff2fbd,#9d4dff);box-shadow:0 0 5px #ff2fbd}.vle-mode-sk.sk-candy i:first-child{height:10px;border-radius:4px 4px 18px 18px}.vle-mode-sk.sk-candy i:last-child{width:58%;background:linear-gradient(90deg,#40ff87,#f6ff45)}",
  ".vle-mode-sk.sk-quilt{flex-direction:column;gap:4px;background:repeating-linear-gradient(45deg,#3b2948 0 9px,#2c2038 9px 18px)}.vle-mode-sk.sk-quilt i{height:6px;border:1px dashed #f1d28a;border-radius:4px 12px 4px 12px;background:linear-gradient(90deg,#f08fc8,#8fdab0)}.vle-mode-sk.sk-quilt i:first-child{height:10px;border-left:4px solid #87bfe8}.vle-mode-sk.sk-quilt i:last-child{width:65%;background:linear-gradient(90deg,#aa89d4,#87bfe8)}",
  ...
  "html[data-vle-chrome='atomic'][data-shape-present='atomic-console'] .vld-pc-item{position:relative;z-index:2;color:#fff1c9;background:#174a45;border-color:color-mix(in srgb,var(--vg2) 72%,#174a45);text-shadow:none}",
  ...
  "html[data-vle-chrome='nocturne'] .vle-mark::after{content:' \\2726';margin-left:7px;color:var(--vg);font:600 .52em/1 var(--vmono);letter-spacing:.13em;font-style:normal}",
  ...
  "html[data-vle-chrome='nocturne'] .vlf-frame{border-radius:34px 34px 8px 8px;border:1px solid color-mix(in srgb,var(--vg) 52%,transparent);box-shadow:inset 0 0 0 6px color-mix(in srgb,var(--vg) 5%,transparent),0 24px 60px rgba(0,0,0,.66)}",
  "html[data-vle-chrome='nocturne'] .vlf-bar{min-height:58px;box-sizing:border-box;padding-inline:18px;background:linear-gradient(90deg,var(--vsurf-2),color-mix(in srgb,var(--vg2) 40%,var(--vsurf-2)),var(--vsurf-2));border-bottom:3px double var(--vg);color:var(--vg);font-family:var(--vserif);font-style:italic}",
  "html[data-vle-chrome='nocturne'][data-shape-present='nocturne-stage'] .vld-pc{background:radial-gradient(ellipse at 50% 18%,color-mix(in srgb,var(--vg2) 45%,var(--vsurf-1)) 0 8%,transparent 42%),linear-gradient(115deg,var(--vsurf-2),var(--vsurf-1),var(--vsurf-2));text-align:left}",
  ...
  "html[data-vle-chrome='nocturne'][data-shape-items='nocturne-reliquary'] .vle-item-row{background:radial-gradient(ellipse at 50% 90%,color-mix(in srgb,var(--vg2) 35%,transparent),transparent 46%),var(--vsurf-2);text-align:left;border-bottom:4px double var(--vg)}",
  ...
  // ROCOCO — a pastel porcelain salon. Shape remains in the border and padding;
  // every prose-bearing surface keeps a conventional full content rectangle.
  "html[data-vle-chrome='rococo'] .vle-root{font-family:var(--vserif);background-image:radial-gradient(circle at 14% 9%,color-mix(in srgb,var(--vg) 18%,transparent),transparent 18%),radial-gradient(circle at 86% 16%,color-mix(in srgb,var(--v-pos) 17%,transparent),transparent 22%),repeating-linear-gradient(0deg,transparent 0 31px,color-mix(in srgb,var(--vg) 3%,transparent) 31px 32px);background-size:auto,auto,32px 32px}",
  "html[data-vle-chrome='rococo'] .vle-navpanel{border:3px double color-mix(in srgb,var(--vg) 72%,transparent);border-radius:42px 12px 42px 12px;background:linear-gradient(145deg,color-mix(in srgb,var(--vg) 13%,var(--vsurf-1)),color-mix(in srgb,var(--v-pos) 12%,var(--vsurf-2)));box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 12%,transparent),inset 0 0 0 6px color-mix(in srgb,var(--vg) 5%,transparent),0 18px 34px rgba(0,0,0,.25)}",
  "html[data-vle-chrome='rococo'] .vle-head,html[data-vle-chrome='rococo'] .vlf-title{font-family:var(--vserif);font-style:italic;letter-spacing:.045em;color:var(--vg)}",
  "html[data-vle-chrome='rococo'] .vle-mark::after{content:' \\2766';margin-left:7px;color:var(--v-pos);font-style:normal}",
  "html[data-vle-chrome='rococo'] .vle-card,html[data-vle-chrome='rococo'] .vle-rel-card,html[data-vle-chrome='rococo'] .vld-sec,html[data-vle-chrome='rococo'] .vld-pc,html[data-vle-chrome='rococo'] .vle-item-row{position:relative;text-align:left;border-color:color-mix(in srgb,var(--vg) 68%,transparent);background:linear-gradient(145deg,color-mix(in srgb,var(--vg) 8%,var(--vsurf-1)),color-mix(in srgb,var(--v-pos) 6%,var(--vsurf-2)));box-shadow:inset 0 0 0 4px color-mix(in srgb,var(--vg) 5%,transparent),0 10px 24px rgba(0,0,0,.2)}",
  "html[data-vle-chrome='rococo'] .vld-h,html[data-vle-chrome='rococo'] .vle-sec-h,html[data-vle-chrome='rococo'] .vle-card-name,html[data-vle-chrome='rococo'] .vle-rel-n{font-family:var(--vserif);font-style:italic;color:var(--vg);letter-spacing:.025em}",
  "html[data-vle-chrome='rococo'] .vle-tabbtn{border-radius:20px 6px 20px 6px;font-family:var(--vserif);text-transform:none;letter-spacing:.035em}",
  "html[data-vle-chrome='rococo'] .vle-tabbtn.on{color:var(--vi);border-color:var(--vg);background:linear-gradient(120deg,color-mix(in srgb,var(--vg) 24%,var(--vsurf-1)),color-mix(in srgb,var(--v-pos) 22%,var(--vsurf-1)));box-shadow:inset 0 0 0 2px color-mix(in srgb,var(--vg) 16%,transparent)}",
  "html[data-vle-chrome='rococo'] .vle-av,html[data-vle-chrome='rococo'] .vld-pc-av{border-radius:50%;border:3px double var(--vg);background:radial-gradient(circle at 38% 30%,color-mix(in srgb,var(--v-info) 28%,var(--vsurf-1)),var(--vsurf-2));box-shadow:0 0 0 3px color-mix(in srgb,var(--v-pos) 20%,transparent)}",
  "html[data-vle-chrome='rococo'] .v-chip{border-radius:14px 5px 14px 5px;border-color:color-mix(in srgb,var(--vg) 48%,transparent)}",
  "html[data-vle-chrome='rococo'] .vlf-frame{border-radius:40px 12px 40px 12px;border:3px double color-mix(in srgb,var(--vg) 72%,transparent);box-shadow:inset 0 0 0 6px color-mix(in srgb,var(--v-pos) 6%,transparent),0 24px 60px rgba(0,0,0,.48)}",
  "html[data-vle-chrome='rococo'] .vlf-bar{min-height:56px;box-sizing:border-box;padding-inline:20px;background:linear-gradient(90deg,color-mix(in srgb,var(--vg) 15%,var(--vsurf-2)),color-mix(in srgb,var(--v-pos) 16%,var(--vsurf-1)),color-mix(in srgb,var(--vg) 15%,var(--vsurf-2)));border-bottom:3px double var(--vg);color:var(--vg);font-family:var(--vserif);font-style:italic;text-align:left}",
  "html[data-vle-chrome='rococo'][data-shape-present='rococo-shell'] .vld-pc{background:radial-gradient(ellipse at 50% 0,color-mix(in srgb,var(--vg) 24%,transparent),transparent 36%),linear-gradient(145deg,var(--vsurf-1),color-mix(in srgb,var(--v-pos) 9%,var(--vsurf-2)))}",
  "html[data-vle-chrome='rococo'][data-shape-bonds='rococo-cartouche'] .vle-rel-card{border-color:var(--v-pos);background:linear-gradient(105deg,color-mix(in srgb,var(--v-pos) 16%,var(--vsurf-1)),var(--vsurf-2),color-mix(in srgb,var(--vg) 11%,var(--vsurf-1)))}",
  "html[data-vle-chrome='rococo'][data-shape-cast='rococo-cameo'] .vle-card:not(.vle-fac){border-color:color-mix(in srgb,var(--vg) 82%,var(--v-pos));background:radial-gradient(circle 4px at 12px 12px,var(--v-pos) 88%,transparent 92%),radial-gradient(circle 4px at calc(100% - 12px) 12px,var(--v-pos) 88%,transparent 92%),radial-gradient(circle 4px at 12px calc(100% - 12px),var(--vg) 88%,transparent 92%),radial-gradient(circle 4px at calc(100% - 12px) calc(100% - 12px),var(--vg) 88%,transparent 92%),linear-gradient(90deg,color-mix(in srgb,var(--vg) 13%,var(--vsurf-1)),var(--vsurf-1) 52%,color-mix(in srgb,var(--v-pos) 13%,var(--vsurf-2)));box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 10%,transparent),inset 0 0 0 6px color-mix(in srgb,var(--vg) 9%,transparent),0 10px 24px rgba(0,0,0,.2)}",
  "html[data-vle-chrome='rococo'][data-shape-beats='rococo-ribbon'] .vle-mem--beat{background:linear-gradient(90deg,color-mix(in srgb,var(--vg) 18%,var(--vsurf-1)),var(--vsurf-1) 22%,var(--vsurf-2))}",
  "html[data-vle-chrome='rococo'][data-shape-factions='rococo-cabinet'] .vle-fac{border-color:var(--v-pos);background:linear-gradient(145deg,color-mix(in srgb,var(--v-pos) 18%,var(--vsurf-1)),var(--vsurf-2))}",
  "html[data-vle-chrome='rococo'][data-shape-items='rococo-medallion'] .vle-item-row{border-color:var(--v-info);background:radial-gradient(ellipse at 50% 100%,color-mix(in srgb,var(--v-warn) 14%,transparent),transparent 48%),var(--vsurf-1)}",

  // CANDY ALCHEMIST — fluorescent glassware on a black-light bench.
  "html[data-vle-chrome='candy'] .vle-root{font-family:var(--vserif);background-image:radial-gradient(circle at 12px 12px,color-mix(in srgb,var(--v-pos) 52%,transparent) 0 1.5px,transparent 3px),radial-gradient(circle at 82% 16%,color-mix(in srgb,var(--vg) 22%,transparent),transparent 18%),radial-gradient(circle at 18% 72%,color-mix(in srgb,var(--v-info) 19%,transparent),transparent 22%),linear-gradient(color-mix(in srgb,var(--v-info) 9%,transparent) 1px,transparent 1px),linear-gradient(90deg,color-mix(in srgb,var(--vg) 9%,transparent) 1px,transparent 1px);background-size:24px 24px,auto,auto,32px 32px,32px 32px;box-shadow:inset 0 0 72px color-mix(in srgb,var(--vg) 9%,transparent),inset 0 0 28px color-mix(in srgb,var(--v-info) 8%,transparent)}",
  "html[data-vle-chrome='candy'] .vle-navpanel{border:2px solid var(--v-info);border-radius:12px 34px 12px 34px;background:linear-gradient(145deg,color-mix(in srgb,var(--vg) 10%,var(--vsurf-1)),var(--vsurf-2));box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 13%,transparent),0 0 26px color-mix(in srgb,var(--vg) 26%,transparent),0 18px 38px rgba(0,0,0,.38)}",
  "html[data-vle-chrome='candy'] .vle-head,html[data-vle-chrome='candy'] .vlf-title{font-family:var(--vserif);font-weight:800;letter-spacing:.045em;color:var(--vg);text-shadow:0 0 12px color-mix(in srgb,var(--vg) 65%,transparent)}",
  "html[data-vle-chrome='candy'] .vle-card,html[data-vle-chrome='candy'] .vle-rel-card,html[data-vle-chrome='candy'] .vld-sec,html[data-vle-chrome='candy'] .vld-pc,html[data-vle-chrome='candy'] .vle-item-row{position:relative;text-align:left;border-color:var(--v-info);background:linear-gradient(145deg,color-mix(in srgb,var(--vg) 13%,var(--vsurf-1)),color-mix(in srgb,var(--v-info) 10%,var(--vsurf-2)));box-shadow:inset 0 0 16px color-mix(in srgb,var(--vg) 10%,transparent),0 0 9px color-mix(in srgb,var(--v-pos) 35%,transparent),0 0 19px color-mix(in srgb,var(--v-info) 32%,transparent),0 10px 24px rgba(0,0,0,.32)}",
  "html[data-vle-chrome='candy'] .vld-h,html[data-vle-chrome='candy'] .vle-sec-h,html[data-vle-chrome='candy'] .vle-card-name,html[data-vle-chrome='candy'] .vle-rel-n{font-family:var(--vserif);font-weight:800;color:var(--v-info);letter-spacing:.035em;text-shadow:0 0 8px color-mix(in srgb,var(--v-info) 45%,transparent)}",
  "html[data-vle-chrome='candy'] .vle-tabbtn{border-radius:999px 12px 999px 12px;border-color:color-mix(in srgb,var(--v-info) 45%,transparent);font-weight:800;letter-spacing:.04em}",
  "html[data-vle-chrome='candy'] .vle-tabbtn.on{color:var(--vi);border-color:var(--vg);background:linear-gradient(105deg,color-mix(in srgb,var(--vg) 40%,var(--vsurf-1)),color-mix(in srgb,var(--v-info) 28%,var(--vsurf-1)));box-shadow:0 0 14px color-mix(in srgb,var(--vg) 42%,transparent)}",
  "html[data-vle-chrome='candy'] .vle-av,html[data-vle-chrome='candy'] .vld-pc-av{border-radius:50%;border:2px solid var(--v-pos);background:radial-gradient(circle at 35% 30%,color-mix(in srgb,var(--v-warn) 58%,#fff),color-mix(in srgb,var(--vg) 62%,var(--vsurf-2)));box-shadow:0 0 0 3px color-mix(in srgb,var(--v-info) 32%,transparent),0 0 15px color-mix(in srgb,var(--v-pos) 36%,transparent)}",
  "html[data-vle-chrome='candy'] .v-chip,html[data-vle-chrome='candy'] .vle-btn,html[data-vle-chrome='candy'] .vle-iconbtn{border-radius:999px;border-color:var(--v-pos);box-shadow:inset 0 0 8px color-mix(in srgb,var(--v-pos) 16%,transparent),0 0 8px color-mix(in srgb,var(--vg) 26%,transparent)}",
  "html[data-vle-chrome='candy'] .vlf-frame{border-radius:14px 38px 14px 38px;border:2px solid var(--v-info);box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 12%,transparent),0 0 28px color-mix(in srgb,var(--vg) 28%,transparent),0 24px 60px rgba(0,0,0,.58)}",
  "html[data-vle-chrome='candy'] .vlf-bar{min-height:56px;box-sizing:border-box;padding-inline:19px;background:linear-gradient(90deg,color-mix(in srgb,var(--vg) 38%,var(--vsurf-2)),color-mix(in srgb,var(--v-info) 25%,var(--vsurf-1)),color-mix(in srgb,var(--v-pos) 24%,var(--vsurf-2)));border-bottom:2px solid var(--v-info);color:var(--vi);font-weight:800;text-align:left}",
  "html[data-vle-chrome='candy'][data-shape-present='candy-vessel'] .vld-pc{background:radial-gradient(ellipse at 50% 100%,color-mix(in srgb,var(--vg) 32%,transparent),transparent 60%),var(--vsurf-2)}",
  "html[data-vle-chrome='candy'][data-shape-bonds='candy-flask'] .vle-rel-card{border-color:var(--v-pos);background:linear-gradient(90deg,color-mix(in srgb,var(--v-info) 14%,var(--vsurf-1)) 0 49%,var(--vsurf-2) 49% 51%,color-mix(in srgb,var(--vg) 15%,var(--vsurf-1)) 51%)}",
  "html[data-vle-chrome='candy'][data-shape-cast='candy-capsule'] .vle-card:not(.vle-fac){border-color:var(--vg);background:linear-gradient(90deg,color-mix(in srgb,var(--vg) 20%,var(--vsurf-1)) 0 48%,color-mix(in srgb,var(--v-info) 18%,var(--vsurf-2)) 52%);box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 16%,transparent),inset 0 0 18px color-mix(in srgb,var(--vg) 16%,transparent),0 0 12px color-mix(in srgb,var(--vg) 48%,transparent),0 0 23px color-mix(in srgb,var(--v-info) 38%,transparent)}",
  "html[data-vle-chrome='candy'][data-shape-beats='candy-rack'] .vle-mem--beat{border-color:var(--v-warn);background:linear-gradient(90deg,color-mix(in srgb,var(--v-pos) 18%,var(--vsurf-1)),var(--vsurf-2))}",
  "html[data-vle-chrome='candy'][data-shape-factions='candy-drawer'] .vle-fac{border-color:var(--vg);background:repeating-linear-gradient(0deg,color-mix(in srgb,var(--v-info) 8%,var(--vsurf-1)) 0 18px,var(--vsurf-2) 18px 36px)}",
  "html[data-vle-chrome='candy'][data-shape-items='candy-lozenge'] .vle-item-row{border-color:var(--v-pos);background:linear-gradient(105deg,color-mix(in srgb,var(--v-pos) 18%,var(--vsurf-1)),color-mix(in srgb,var(--v-warn) 10%,var(--vsurf-2)))}",
  "html[data-vle-chrome='candy'] .vle-tabbtn,html[data-vle-chrome='candy'] .vle-sec-h,html[data-vle-chrome='candy'] .vle-card-sub,html[data-vle-chrome='candy'] .vle-card-disp,html[data-vle-chrome='candy'] .vld-meta{filter:drop-shadow(0 0 4px color-mix(in srgb,var(--v-info) 35%,transparent))}",
  "html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vle-navpanel,html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vle-card,html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vle-rel-card,html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vld-pc,html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vle-item-row{animation:vle-candy-glow 4.8s ease-in-out infinite}",
  "html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vle-card:nth-child(2n),html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vle-rel-card:nth-child(2n),html[data-vle-chrome='candy']:not([data-vle-motion='off']) .vle-item-row:nth-child(2n){animation-delay:-2.4s}",
  "html[data-vle-chrome='candy'][data-shape-cast='candy-capsule']:not([data-vle-motion='off']) .vle-card:not(.vle-fac){animation:vle-candy-cast-glow 4.8s ease-in-out infinite}",
  "@keyframes vle-candy-glow{0%,100%{filter:brightness(1);box-shadow:inset 0 0 16px color-mix(in srgb,var(--vg) 10%,transparent),0 0 9px color-mix(in srgb,var(--v-pos) 35%,transparent),0 0 19px color-mix(in srgb,var(--v-info) 32%,transparent),0 10px 24px rgba(0,0,0,.32)}50%{filter:brightness(1.12);box-shadow:inset 0 0 20px color-mix(in srgb,var(--vg) 17%,transparent),0 0 14px color-mix(in srgb,var(--v-pos) 52%,transparent),0 0 30px color-mix(in srgb,var(--v-info) 48%,transparent),0 10px 24px rgba(0,0,0,.32)}}",
  "@keyframes vle-candy-cast-glow{0%,100%{filter:brightness(1);box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 16%,transparent),inset 0 0 18px color-mix(in srgb,var(--vg) 16%,transparent),0 0 12px color-mix(in srgb,var(--vg) 48%,transparent),0 0 23px color-mix(in srgb,var(--v-info) 38%,transparent)}50%{filter:brightness(1.14);box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 25%,transparent),inset 0 0 25px color-mix(in srgb,var(--vg) 24%,transparent),0 0 18px color-mix(in srgb,var(--vg) 62%,transparent),0 0 34px color-mix(in srgb,var(--v-info) 54%,transparent)}}",
  "@media (prefers-reduced-motion:reduce){html[data-vle-chrome='candy'] .vle-navpanel,html[data-vle-chrome='candy'] .vle-card,html[data-vle-chrome='candy'] .vle-rel-card,html[data-vle-chrome='candy'] .vld-pc,html[data-vle-chrome='candy'] .vle-item-row{animation:none!important}}",

  // DREAM QUILT — textile surfaces and slow optional motion. Animations are quiet,
  // scoped, and fully disabled by both VELLUM's motion toggle and OS preference.
  "html[data-vle-chrome='quilt'] .vle-root{font-family:var(--vserif);background-image:repeating-linear-gradient(45deg,color-mix(in srgb,var(--vg) 8%,transparent) 0 16px,color-mix(in srgb,var(--v-pos) 8%,transparent) 16px 32px,color-mix(in srgb,var(--v-info) 8%,transparent) 32px 48px,color-mix(in srgb,var(--v-warn) 8%,transparent) 48px 64px),repeating-linear-gradient(-45deg,transparent 0 31px,color-mix(in srgb,var(--v-warn) 9%,transparent) 31px 32px);background-size:96px 96px,64px 64px}",
  "html[data-vle-chrome='quilt'] .vle-navpanel{border:2px dashed color-mix(in srgb,var(--v-warn) 75%,transparent);border-radius:9px;background:linear-gradient(145deg,color-mix(in srgb,var(--vg) 12%,var(--vsurf-1)),color-mix(in srgb,var(--v-info) 10%,var(--vsurf-2)));box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 12%,transparent),inset 0 0 0 6px color-mix(in srgb,var(--v-warn) 8%,transparent),0 18px 36px rgba(0,0,0,.28)}",
  "html[data-vle-chrome='quilt'] .vle-head,html[data-vle-chrome='quilt'] .vlf-title{font-family:var(--vserif);font-weight:700;letter-spacing:.035em;color:var(--vg)}",
  "html[data-vle-chrome='quilt'] .vle-mark::after{content:' \\273D';margin-left:7px;color:var(--v-warn)}",
  "html[data-vle-chrome='quilt'] .vle-card,html[data-vle-chrome='quilt'] .vle-rel-card,html[data-vle-chrome='quilt'] .vld-sec,html[data-vle-chrome='quilt'] .vld-pc,html[data-vle-chrome='quilt'] .vle-item-row{position:relative;text-align:left;border-color:color-mix(in srgb,var(--v-warn) 72%,transparent);background:linear-gradient(135deg,color-mix(in srgb,var(--vg) 10%,var(--vsurf-1)),color-mix(in srgb,var(--v-info) 8%,var(--vsurf-2)));box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 7%,transparent),0 11px 25px rgba(0,0,0,.22)}",
  "html[data-vle-chrome='quilt'] .vld-h,html[data-vle-chrome='quilt'] .vle-sec-h,html[data-vle-chrome='quilt'] .vle-card-name,html[data-vle-chrome='quilt'] .vle-rel-n{font-family:var(--vserif);font-weight:700;color:var(--vg);letter-spacing:.025em}",
  "html[data-vle-chrome='quilt'] .vle-tabbtn{border-radius:7px 18px 7px 18px;border-style:dashed;font-family:var(--vserif);text-transform:none}",
  "html[data-vle-chrome='quilt'] .vle-tabbtn.on{color:var(--vi);border-color:var(--v-warn);background:linear-gradient(105deg,color-mix(in srgb,var(--vg) 24%,var(--vsurf-1)),color-mix(in srgb,var(--v-pos) 20%,var(--vsurf-1)));box-shadow:inset 0 0 0 2px color-mix(in srgb,var(--v-warn) 20%,transparent)}",
  "html[data-vle-chrome='quilt'] .vle-av,html[data-vle-chrome='quilt'] .vld-pc-av{border-radius:50%;border:2px dashed var(--v-warn);background:radial-gradient(circle at 38% 30%,color-mix(in srgb,var(--v-info) 40%,var(--vsurf-1)),color-mix(in srgb,var(--vg) 35%,var(--vsurf-2)));box-shadow:0 0 0 4px color-mix(in srgb,var(--v-pos) 16%,transparent)}",
  "html[data-vle-chrome='quilt'] .v-chip{border-radius:7px 14px 7px 14px;border-style:dashed}",
  "html[data-vle-chrome='quilt'] .vlf-frame{border-radius:26px 10px 30px 12px;border:2px dashed color-mix(in srgb,var(--v-warn) 78%,transparent);box-shadow:inset 0 0 0 5px color-mix(in srgb,var(--v-pos) 9%,transparent),0 24px 60px rgba(0,0,0,.5)}",
  "html[data-vle-chrome='quilt'] .vlf-bar{min-height:56px;box-sizing:border-box;padding-inline:19px;background:linear-gradient(90deg,color-mix(in srgb,var(--vg) 22%,var(--vsurf-2)),color-mix(in srgb,var(--v-pos) 19%,var(--vsurf-1)),color-mix(in srgb,var(--v-info) 20%,var(--vsurf-2)));border-bottom:2px dashed var(--v-warn);color:var(--vi);font-family:var(--vserif);text-align:left}",
  "html[data-vle-chrome='quilt'][data-shape-present='quilt-patch'] .vld-pc{border-color:var(--vg);background:linear-gradient(122deg,color-mix(in srgb,var(--vg) 17%,var(--vsurf-1)),var(--vsurf-2) 55%,color-mix(in srgb,var(--v-pos) 10%,var(--vsurf-1)))}",
  "html[data-vle-chrome='quilt'][data-shape-bonds='quilt-join'] .vle-rel-card{border-color:var(--v-info);background:linear-gradient(90deg,color-mix(in srgb,var(--v-info) 14%,var(--vsurf-1)) 0 49%,color-mix(in srgb,var(--v-warn) 18%,var(--vsurf-2)) 49% 51%,color-mix(in srgb,var(--v-pos) 14%,var(--vsurf-1)) 51%)}",
  "html[data-vle-chrome='quilt'][data-shape-cast='quilt-locket'] .vle-card:not(.vle-fac){border-color:var(--v-warn);background:linear-gradient(90deg,color-mix(in srgb,var(--vg) 15%,var(--vsurf-1)) 0 31%,color-mix(in srgb,var(--v-pos) 12%,var(--vsurf-1)) 31% 33%,var(--vsurf-2) 33% 67%,color-mix(in srgb,var(--v-info) 12%,var(--vsurf-2)) 67% 69%,color-mix(in srgb,var(--v-warn) 12%,var(--vsurf-2)) 69%);box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-warn) 12%,transparent),inset 0 0 0 6px color-mix(in srgb,var(--v-pos) 8%,transparent),0 11px 25px rgba(0,0,0,.22)}",
  "html[data-vle-chrome='quilt'][data-shape-beats='quilt-runner'] .vle-mem--beat{border-color:var(--v-pos);background:linear-gradient(105deg,color-mix(in srgb,var(--v-pos) 14%,var(--vsurf-1)),var(--vsurf-2))}",
  "html[data-vle-chrome='quilt'][data-shape-factions='quilt-pocket'] .vle-fac{border-color:var(--vg);background:linear-gradient(180deg,color-mix(in srgb,var(--vg) 13%,var(--vsurf-1)),var(--vsurf-2))}",
  "html[data-vle-chrome='quilt'][data-shape-items='quilt-square'] .vle-item-row{border-color:var(--v-warn);background:linear-gradient(135deg,color-mix(in srgb,var(--v-warn) 13%,var(--vsurf-1)),color-mix(in srgb,var(--v-info) 11%,var(--vsurf-2)))}",
  "@keyframes vle-quilt-drift{0%,100%{background-position:0 0}50%{background-position:18px 10px}}",
  "@keyframes vle-quilt-breathe{0%,100%{box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--v-pos) 7%,transparent),0 11px 25px rgba(0,0,0,.22)}50%{box-shadow:inset 0 0 0 3px color-mix(in srgb,var(--vg) 12%,transparent),0 12px 29px color-mix(in srgb,var(--vg) 14%,transparent)}}",
  "html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vle-root{animation:vle-quilt-drift 18s ease-in-out infinite}",
  "html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vle-card,html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vle-rel-card,html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vld-pc,html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vle-item-row{animation:vle-quilt-breathe 7s ease-in-out infinite}",
  "html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vle-card:nth-child(2n),html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vle-rel-card:nth-child(2n),html[data-vle-chrome='quilt']:not([data-vle-motion='off']) .vle-item-row:nth-child(2n){animation-delay:-3.5s}",
  "@media (prefers-reduced-motion:reduce){html[data-vle-chrome='quilt'] .vle-root,html[data-vle-chrome='quilt'] .vle-card,html[data-vle-chrome='quilt'] .vle-rel-card,html[data-vle-chrome='quilt'] .vld-pc,html[data-vle-chrome='quilt'] .vle-item-row{animation:none!important}}",
  ...
  // ROCOCO object cards ------------------------------------------------------
  ...shapeDetail('rococo-shell', "content:'';position:absolute;top:5px;left:18%;right:18%;height:11px;pointer-events:none;z-index:1;background:radial-gradient(ellipse at 50% 100%,transparent 0 42%,var(--vg) 44% 50%,transparent 52%),radial-gradient(circle 2px at 25% 65%,var(--v-pos) 92%,transparent),radial-gradient(circle 2px at 75% 65%,var(--v-pos) 92%,transparent);opacity:.7", '::before'),
  ...shapeDetail('rococo-cartouche', "content:'';position:absolute;inset:5px 8px;pointer-events:none;z-index:1;border:3px double color-mix(in srgb,var(--vg) 52%,transparent);border-radius:inherit;background:radial-gradient(circle 2px at 4px 50%,var(--v-pos) 90%,transparent),radial-gradient(circle 2px at calc(100% - 4px) 50%,var(--v-pos) 90%,transparent);opacity:.75", '::before'),
  // Cast: an heirloom portrait label with four porcelain rosettes, kept inside a
  // readable rectangular record card instead of a wide oval.
  ...shapeDetail('rococo-cameo', "content:'';position:absolute;inset:6px 9px;pointer-events:none;z-index:1;border:3px double color-mix(in srgb,var(--vg) 48%,transparent);border-radius:8px 8px 4px 4px;background:radial-gradient(circle 3px at 8px 8px,var(--v-pos) 90%,transparent),radial-gradient(circle 3px at calc(100% - 8px) 8px,var(--v-pos) 90%,transparent),radial-gradient(circle 3px at 8px calc(100% - 8px),var(--vg) 90%,transparent),radial-gradient(circle 3px at calc(100% - 8px) calc(100% - 8px),var(--vg) 90%,transparent),linear-gradient(110deg,transparent 20%,color-mix(in srgb,var(--v-info) 11%,transparent) 38%,transparent 56%);opacity:.78", '::before'),
  ...shapeDetail('rococo-ribbon', "content:'';position:absolute;top:6px;bottom:6px;left:8px;width:8px;pointer-events:none;z-index:1;border-block:1px solid var(--vg);background:radial-gradient(circle 2px at 50% 50%,var(--v-pos) 92%,transparent);opacity:.72", '::before'),
  ...shapeDetail('rococo-cabinet', "content:'';position:absolute;inset:5px;pointer-events:none;z-index:1;border:3px double color-mix(in srgb,var(--v-pos) 48%,transparent);border-radius:8px 8px 3px 3px;background:radial-gradient(circle 2px at 50% 8px,var(--vg) 92%,transparent);opacity:.72", '::before'),
  ...shapeDetail('rococo-medallion', "content:'';position:absolute;inset:6px 9px 9px;pointer-events:none;z-index:1;border:1px solid color-mix(in srgb,var(--vg) 48%,transparent);border-radius:20px 20px 48% 48%/16px 16px 25% 25%;box-shadow:inset 0 -5px 0 color-mix(in srgb,var(--v-warn) 8%,transparent);opacity:.76", '::before'),
  // CANDY ALCHEMIST object cards --------------------------------------------
  ...shapeDetail('candy-vessel', "content:'';position:absolute;top:4px;left:26%;right:26%;height:10px;pointer-events:none;z-index:1;border-inline:2px solid var(--v-info);background:linear-gradient(90deg,var(--v-info),var(--v-info)) 50% 2px/70% 2px no-repeat;box-shadow:0 0 8px color-mix(in srgb,var(--v-info) 55%,transparent);opacity:.85", '::before'),
  ...shapeDetail('candy-flask', "content:'';position:absolute;top:5px;left:calc(50% - 29px);width:58px;height:11px;pointer-events:none;z-index:1;background:linear-gradient(var(--v-info),var(--v-info)) 8px 0/2px 10px no-repeat,linear-gradient(var(--vg),var(--vg)) 48px 0/2px 10px no-repeat,radial-gradient(circle 4px at 9px 9px,var(--v-info) 90%,transparent),radial-gradient(circle 4px at 49px 9px,var(--vg) 90%,transparent);filter:drop-shadow(0 0 5px var(--vg));opacity:.82", '::before'),
  // Cast: a boxed reagent label with an illuminated split seam and two light rails.
  ...shapeDetail('candy-capsule', "content:'';position:absolute;left:9px;right:9px;top:6px;bottom:6px;pointer-events:none;z-index:1;border:1px solid color-mix(in srgb,var(--v-info) 68%,transparent);border-radius:4px;background:linear-gradient(90deg,transparent 49.6%,var(--vg) 49.7% 50.3%,transparent 50.4%),linear-gradient(90deg,var(--v-pos),var(--v-info)) 12px 6px/calc(100% - 24px) 2px no-repeat,linear-gradient(90deg,var(--v-info),var(--vg)) 12px calc(100% - 8px)/calc(100% - 24px) 2px no-repeat;box-shadow:inset 0 0 10px color-mix(in srgb,var(--vg) 12%,transparent),0 0 9px color-mix(in srgb,var(--vg) 55%,transparent);opacity:.78", '::before'),
  ...shapeDetail('candy-rack', "content:'';position:absolute;left:12px;right:12px;bottom:5px;height:8px;pointer-events:none;z-index:1;background:radial-gradient(circle 3px at 8% 0,var(--v-pos) 92%,transparent),radial-gradient(circle 3px at 29% 0,var(--v-info) 92%,transparent),radial-gradient(circle 3px at 50% 0,var(--vg) 92%,transparent),radial-gradient(circle 3px at 71% 0,var(--v-warn) 92%,transparent),radial-gradient(circle 3px at 92% 0,var(--v-pos) 92%,transparent);filter:drop-shadow(0 0 4px var(--v-info));opacity:.82", '::after'),
  ...shapeDetail('candy-drawer', "content:'';position:absolute;inset:5px;pointer-events:none;z-index:1;border:1px solid color-mix(in srgb,var(--v-info) 66%,transparent);border-radius:5px;background:linear-gradient(var(--vg),var(--vg)) 50% 6px/34px 3px no-repeat;box-shadow:inset 0 0 10px color-mix(in srgb,var(--vg) 9%,transparent);opacity:.78", '::before'),
  ...shapeDetail('candy-lozenge', "content:'';position:absolute;right:12px;top:50%;width:26px;height:12px;transform:translateY(-50%);pointer-events:none;z-index:1;background:radial-gradient(circle 3px at 3px 50%,var(--v-pos) 92%,transparent),radial-gradient(circle 3px at 13px 1px,var(--v-info) 92%,transparent),radial-gradient(circle 3px at 23px 50%,var(--vg) 92%,transparent),linear-gradient(30deg,var(--v-warn),var(--v-warn)) 4px 6px/19px 1px no-repeat;filter:drop-shadow(0 0 4px var(--vg));opacity:.72", '::after'),
  // DREAM QUILT object cards -------------------------------------------------
  ...shapeDetail('quilt-patch', "content:'';position:absolute;inset:5px;pointer-events:none;z-index:1;border:1px dashed color-mix(in srgb,var(--v-warn) 68%,transparent);border-radius:inherit;background:linear-gradient(90deg,var(--v-info),var(--v-info)) 7px 7px/11px 3px no-repeat,linear-gradient(90deg,var(--v-pos),var(--v-pos)) calc(100% - 18px) calc(100% - 9px)/11px 3px no-repeat;opacity:.72", '::before'),
  ...shapeDetail('quilt-join', "content:'';position:absolute;top:6px;bottom:6px;left:50%;width:8px;transform:translateX(-50%);pointer-events:none;z-index:1;background:linear-gradient(90deg,transparent 3px,var(--v-warn) 3px 5px,transparent 5px),repeating-linear-gradient(45deg,transparent 0 5px,var(--vg) 5px 6px,transparent 6px 10px);opacity:.62", '::before'),
  // Cast: a stitched sampler label; patch seams and thread knots decorate the
  // safe margin while the character text stays in a conventional rectangle.
  ...shapeDetail('quilt-locket', "content:'';position:absolute;inset:6px 10px;pointer-events:none;z-index:1;border:2px dashed color-mix(in srgb,var(--v-warn) 60%,transparent);border-radius:3px;background:radial-gradient(circle 3px at 10px 9px,var(--vg) 92%,transparent),radial-gradient(circle 3px at calc(100% - 10px) 9px,var(--v-pos) 92%,transparent),radial-gradient(circle 3px at 10px calc(100% - 9px),var(--v-info) 92%,transparent),radial-gradient(circle 3px at calc(100% - 10px) calc(100% - 9px),var(--vg) 92%,transparent),repeating-linear-gradient(90deg,transparent 0 12px,color-mix(in srgb,var(--v-warn) 38%,transparent) 12px 13px) 0 50%/100% 1px no-repeat;opacity:.72", '::before'),
  ...shapeDetail('quilt-runner', "content:'';position:absolute;left:15px;right:15px;top:6px;bottom:6px;pointer-events:none;z-index:1;border-block:2px dashed color-mix(in srgb,var(--v-pos) 62%,transparent);border-radius:18px 4px 18px 4px;opacity:.68", '::before'),
  ...shapeDetail('quilt-pocket', "content:'';position:absolute;left:9px;right:9px;top:5px;height:12px;pointer-events:none;z-index:1;border-bottom:2px dashed var(--vg);border-radius:4px 4px 50% 50%;background:radial-gradient(circle 2px at 35% 5px,var(--v-info) 92%,transparent),radial-gradient(circle 2px at 65% 5px,var(--v-pos) 92%,transparent);opacity:.72", '::before'),
  ...shapeDetail('quilt-square', "content:'';position:absolute;inset:6px;pointer-events:none;z-index:1;border:1px dashed color-mix(in srgb,var(--v-warn) 58%,transparent);border-radius:8px;background:linear-gradient(90deg,transparent 49.7%,color-mix(in srgb,var(--v-info) 28%,transparent) 50%,transparent 50.3%),linear-gradient(transparent 49.7%,color-mix(in srgb,var(--v-pos) 28%,transparent) 50%,transparent 50.3%);opacity:.7", '::before'),
  ...
  ".vle-pt-root{padding:12px;font-size:12px;line-height:1.5;color:var(--lumiverse-text,#cdbfa0);display:grid;grid-template-columns:190px 1fr;gap:12px;align-items:start}",
  "@media (max-width:640px){.vle-pt-root{grid-template-columns:1fr}}",
  ".vle-pt-rail{display:flex;flex-direction:column;gap:6px;padding:10px;background:color-mix(in srgb,var(--vg,#d4af37) 4%,transparent);border:1px solid color-mix(in srgb,var(--vg,#d4af37) 20%,transparent);border-radius:6px;position:sticky;top:8px}",
  ".vle-pt-rail-head{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;color:var(--vg,#d4af37);margin-bottom:4px}",
  ...
  ".vle-pt-rank{width:18px;display:inline-block;font-family:ui-monospace,'Cascadia Code',Consolas,monospace;font-size:10px;color:var(--vg,#d4af37);opacity:0.7;flex-shrink:0}",
  ".vle-pt-name{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
  ".vle-pt-total{font-size:15px;font-weight:700;color:var(--lumiverse-text,#cdbfa0)}",
  ".vle-pt-subhead{font-size:10px;font-weight:600;color:var(--vg,#d4af37);opacity:0.8;margin-top:8px;margin-bottom:2px;text-transform:uppercase;letter-spacing:0.4px}",
  ...
  ".vle-pt-toolbar{display:flex;align-items:center;flex-wrap:wrap;gap:8px;min-width:0;padding:2px 0}",
  ".vle-pt-toolbar>span:not(.vle-pt-dot){min-width:0;overflow-wrap:anywhere}",
  ...
  ".vle-pt-btn{max-width:100%;min-width:0;white-space:normal;overflow-wrap:anywhere;background:color-mix(in srgb,var(--vg,#d4af37) 12%,transparent);border:1px solid color-mix(in srgb,var(--vg,#d4af37) 30%,transparent);color:var(--lumiverse-text,#cdbfa0);border-radius:4px;padding:5px 10px;font-size:11px;cursor:pointer;transition:all 0.15s;margin-top:4px}",
  ...
  ".vle-pt-btn:disabled{cursor:not-allowed;opacity:.48;transform:none}",
  ...
  ".vle-pt-line{display:flex;justify-content:space-between;align-items:center;gap:8px;min-width:0;font-size:11px;color:color-mix(in srgb,var(--lumiverse-text,#cdbfa0) 80%,transparent)}",
  ".vle-pt-line>*{min-width:0}.vle-pt-line strong{color:var(--lumiverse-text,#cdbfa0);font-weight:600;overflow-wrap:anywhere;text-align:right}",
  ".vle-pt-issues{margin:0;padding-left:18px;font-size:10px;line-height:1.45;color:color-mix(in srgb,var(--lumiverse-text,#cdbfa0) 78%,transparent)}",
  ...
  ".vle-preset-stat{flex:1;display:flex;flex-direction:column;align-items:center;padding:6px 8px;background:linear-gradient(180deg,color-mix(in srgb,var(--vg,#d4af37) 5%,rgba(16,14,10,.95)),rgba(14,12,9,.97))}",
  ".vle-preset-stat .v{font:700 calc(14px * var(--vscale,1))/1 var(--vmono,ui-monospace,monospace);color:var(--vg,#d4af37);letter-spacing:.3px}",
  ".vle-preset-stat .k{font:500 8px/1.2 var(--vmono,ui-monospace,monospace);text-transform:uppercase;letter-spacing:.5px;color:color-mix(in srgb,var(--vg,#d4af37) 55%,transparent);margin-top:2px}",
  ".vle-preset-stat.ok .v{color:var(--v-pos,#8fa67e)}",
  ".vle-preset-stat.err .v{color:var(--v-neg,#c96a6a)}",
  ...
  ".vle-pv-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr)}",
  ".vle-pv-grid>section{min-width:0}.vle-pv-grid>section+section{border-left:1px solid color-mix(in srgb,var(--vg,#d4af37) 15%,transparent)}",
  ...
  ".vle-prompt-inspect{padding:8px 11px;font-size:10px}.vle-prompt-inspect summary{cursor:pointer;overflow-wrap:anywhere}.vle-prompt-inspect pre{max-height:320px;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;font:10px/1.45 var(--vmono,ui-monospace,monospace)}",
  ".vle-mobile-editor{margin:14px;padding:12px;border:1px solid color-mix(in srgb,var(--vg,#d4af37) 28%,transparent);border-radius:7px;background:color-mix(in srgb,var(--vg,#d4af37) 5%,transparent)}",
  ".vle-mobile-editor [data-pp-loom]{min-width:0;margin-top:10px}.vle-mobile-save{display:flex;align-items:center;justify-content:flex-end;flex-wrap:wrap;gap:9px;margin-top:10px;font-size:10px;color:color-mix(in srgb,var(--lumiverse-text,#cdbfa0) 65%,transparent)}",
  "@media (max-width:700px){.vle-pv-grid{grid-template-columns:1fr}.vle-pv-grid>section+section{border-left:0;border-top:1px solid color-mix(in srgb,var(--vg,#d4af37) 15%,transparent)}.vle-pt-line{align-items:flex-start;flex-wrap:wrap}.vle-pt-line strong{text-align:left}.vle-pv-tag{margin-left:0}.vle-pv-top{flex-wrap:wrap}}",
  ...
  ".vle-lore-history{margin-top:.35rem;max-width:100%;font-size:.72rem;color:var(--vink-faint)}.vle-lore-history p{overflow-wrap:anywhere}.vle-lore-actions{display:flex;align-items:center;justify-content:flex-end;gap:.28rem;white-space:nowrap;flex:0 0 auto;min-width:max-content}.vle-lore-actions .vle-mini{box-sizing:border-box;flex:0 0 auto;overflow:hidden;white-space:nowrap}.vle-lore-actions .vle-lore-btn{display:inline-flex;width:auto;min-width:2.8rem;height:24px;padding:0 .48rem;align-items:center;justify-content:center}.vle-lore-actions .vle-lore-icon{width:24px;min-width:24px;height:24px;padding:0}.vle-lore-turn{font-family:var(--mono);font-size:0.52rem;color:var(--vink-faint);flex-shrink:0;letter-spacing:0.03em;margin-right:.15rem}",
  "@media(max-width:620px){.vle-lore-row{grid-template-columns:1fr}.vle-lore-actions{justify-content:flex-start;flex-wrap:wrap;min-width:0}}",
  ...
  "@keyframes vle-ob-fade-in{from{opacity:0}to{opacity:1}}",
  ".vle-ob-shell{background:var(--vsurf-1);border:2px solid var(--vg);border-radius:12px;max-width:720px;width:100%;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.6);animation:vle-ob-slide-up 0.4s ease}",
  "@keyframes vle-ob-slide-up{from{transform:translateY(30px);opacity:0}to{transform:translateY(0);opacity:1}}",
  ".vle-ob-head{display:flex;align-items:center;justify-content:space-between;padding:1.5rem 2rem;border-bottom:2px solid var(--vg)}",
  ".vle-ob-head h2{margin:0;font-size:1.5rem;color:var(--vg);font-weight:600;letter-spacing:0.02em}",
  ".vle-ob-x{background:transparent;border:none;color:var(--vink-dim);font-size:1.5rem;cursor:pointer;padding:0.25rem 0.5rem;line-height:1;transition:color 0.2s}",
  ".vle-ob-x:hover{color:var(--v-neg)}",
  ".vle-ob-body{flex:1;overflow-y:auto;padding:2rem;color:var(--vink-dim);line-height:1.6}",
  ".vle-ob-body p{margin:0 0 1rem}",
  ".vle-ob-body p:last-child{margin-bottom:0}",
  ".vle-ob-body strong{color:var(--vink);font-weight:600}",
  ".vle-ob-body em{color:var(--vg);font-style:italic}",
  ".vle-ob-body code{font-family:var(--mono);font-size:0.9em;background:color-mix(in srgb,var(--vg) 12%,transparent);border:1px solid color-mix(in srgb,var(--vg) 25%,transparent);padding:0.15em 0.4em;border-radius:3px;color:var(--vg)}",
  ".vle-ob-body h4{margin:1.5rem 0 0.75rem;color:var(--vg);font-size:1.1rem;font-weight:600}",
  ".vle-ob-body ul{margin:0.5rem 0;padding-left:1.5rem}",
  ".vle-ob-body ol{margin:0.5rem 0;padding-left:1.5rem}",
  ".vle-ob-body li{margin:0.4rem 0}",
  ".vle-ob-note{background:color-mix(in srgb,var(--v-info) 10%,transparent);border-left:3px solid var(--v-info);padding:0.75rem 1rem;border-radius:4px;margin:1rem 0;font-size:0.95rem}",
  ".vle-ob-grid{display:grid;gap:1rem;margin:1rem 0}",
  ".vle-ob-item{display:flex;gap:0.75rem;padding:0.75rem;background:color-mix(in srgb,var(--vg) 4%,transparent);border:1px solid color-mix(in srgb,var(--vg) 15%,transparent);border-radius:6px}",
  ".vle-ob-icon{font-size:1.5rem;flex-shrink:0;line-height:1}",
  ".vle-ob-section{margin:1.5rem 0}",
  ".vle-ob-section h4{margin-top:0}",
  ".vle-ob-list{list-style:none;padding:0}",
  ".vle-ob-list li{padding:0.35rem 0 0.35rem 1.5rem;position:relative}",
  ".vle-ob-list li::before{content:'▸';position:absolute;left:0;color:var(--vg)}",
  ".vle-ob-steps{counter-reset:step;list-style:none;padding:0}",
  ".vle-ob-steps li{padding:0.5rem 0 0.5rem 2.5rem;position:relative;counter-increment:step}",
  ".vle-ob-steps li::before{content:counter(step);position:absolute;left:0;top:0.4rem;background:var(--vg);color:var(--vsurf-1);width:1.6rem;height:1.6rem;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:600;font-size:0.85rem}",
  ".vle-ob-tips{display:grid;gap:0.75rem;margin:1rem 0}",
  ".vle-ob-tip{display:flex;gap:0.75rem;padding:0.75rem 1rem;background:color-mix(in srgb,var(--vg) 8%,transparent);border:1px solid color-mix(in srgb,var(--vg) 20%,transparent);border-radius:6px}",
  ".vle-ob-tip-icon{font-size:1.25rem;flex-shrink:0}",
  ".vle-ob-cta{text-align:center;font-size:1.05rem;padding:1.5rem 0 0.5rem;color:var(--vg)}",
  ".vle-ob-foot{border-top:2px solid var(--vg);padding:1.25rem 2rem;display:flex;flex-direction:column;gap:1rem}",
  ".vle-ob-progress{display:flex;gap:0.5rem;justify-content:center}",
  ".vle-ob-progress span{width:2rem;height:2rem;border-radius:50%;border:2px solid color-mix(in srgb,var(--vg) 30%,transparent);display:flex;align-items:center;justify-content:center;font-size:0.75rem;color:var(--vink-faint);transition:all 0.3s}",
  ".vle-ob-progress span.active{background:var(--vg);border-color:var(--vg);color:var(--vsurf-1);font-weight:600;transform:scale(1.15)}",
  ".vle-ob-nav{display:flex;justify-content:space-between;align-items:center}",
  ".vle-ob-btn{padding:0.6rem 1.5rem;border-radius:6px;font-weight:600;cursor:pointer;transition:all 0.2s;font-size:0.95rem;border:2px solid color-mix(in srgb,var(--vg) 40%,transparent);background:transparent;color:var(--vink)}",
