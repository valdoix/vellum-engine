...
export const ParallelItem = z.object({ who: z.string().optional(), where: z.string().optional(), activity: z.string(), note: z.string().optional(), src: z.literal('sim').optional(), sourceSubplotId: z.string().optional() });
...
export const EvOffscreen = z.object({ ...base, kind: z.literal('offscreen.op'), op: z.enum(['new', 'advance', 'resolve']), id: z.string(), name: z.string().optional(), who: z.string().optional(), where: z.string().optional(), locationOp: z.enum(['retain', 'refine', 'move']).optional(), gist: z.string().optional(), thread: z.string().optional(), fill: z.boolean().optional(), pressure: z.number().int().min(0).max(5).optional(), hooks: z.array(z.string()).max(6).optional(), stakes: z.string().optional(), autonomy: z.enum(['personal', 'social', 'faction', 'environment', 'mixed']).optional(), beatKind: z.enum(['progress', 'obstacle', 'consequence', 'bridge', 'resolution']).optional(), impact: z.string().optional(), grounding: z.object({ basis: z.array(z.enum(['scene', 'character', 'location', 'lorebook', 'parallel', 'subplot', 'knowledge', 'relationship', 'intent', 'manual'])).max(10), rationale: z.string(), refs: z.array(z.string()).max(20).optional(), before: z.string().optional(), after: z.string().optional() }).optional(), originParallel: z.object({ turn: z.number().int().nonnegative(), day: z.number().int().nonnegative(), who: z.string().optional(), where: z.string().optional(), activity: z.string() }).optional(), provenance: z.object({ kind: z.enum(['foreground_bridge', 'simulation', 'manual', 'promotion', 'explicit']), sourceThreadId: z.string().optional(), sourceSubplotId: z.string().optional() }).optional(), nextTurn: z.number().int().nonnegative().optional(), nextDay: z.number().int().nonnegative().optional(), nextClock: z.number().int().min(0).max(1439).optional(), deadlineDay: z.number().int().nonnegative().optional(), deadlineClock: z.number().int().min(0).max(1439).optional(), dependsOn: z.array(z.string()).max(20).optional(), blockedBy: z.array(z.string()).max(20).optional(), trigger: z.string().optional() });
