...
PRESENT RULES: list {{user}} first. Keep fields/traits blank unless runtime PERSONA STATE is ON; then ground them in agency-permitted player input/prose and put one exact source quote in evidence. This grants no player authorship. List every named on-stage NPC. NPC thought is first-person private thought using only what that NPC knows. traits contains 2–4 stable traits only when first establishing or genuinely changing them.
...
Reserve ~700 output tokens and shorten prose before risking state. Compile only what the prose established. Emit scene/present plus changed supported fields; omit empty sections except a required parallel:[]. Put {{user}} first; blank unless runtime PERSONA STATE is ON, then grounded fields/traits plus exact evidence. Include every named on-stage NPC with a concise private first-person thought bounded by that NPC's knowledge. Reconcile parallel at final T1: no present actor, no stale origin, exact final where/activity, one row per actor. When Time Continuity is on, require exact HH:MM scene.time and matching 0–1439 scene.clock, and reject any day × 1440 + clock lower than injected T0. Decide the whole compact object before opening <vellum>; once opened, finish valid JSON, literal </vellum>, and nothing after it.
{{/if}}
...
1. CORE SNAPSHOT: write current scene and present from the injected record and this turn. On an active scene, do not omit them to save tokens. When Time Continuity is on, scene.time must be exact zero-padded 24-hour HH:MM and scene.clock must equal HH × 60 + MM; both are mandatory, and a word such as morning/evening is invalid. Compute injected A0 and proposed A1 as day × 1440 + scene.clock; if A1 < A0, the candidate is invalid and must retain T0 or be recomputed from established elapsed time. Put {{user}} first; blank unless runtime PERSONA STATE is ON, then grounded fields/traits plus exact evidence; state must not invent player behavior.
