...
Active scenes require matching HH:MM/clock, e.g. "time":"07:45","clock":465. Put {{user}} first: blank unless PERSONA STATE is ON; then always populate mood, condition, doing, private first-person thought, and stable traits. List every named on-stage NPC with a concise first-person thought limited to their knowledge. On turn 1 or an empty ledger, create one supported actionable unresolved thread and a clear parent arc; set thread.arc to the exact arc title. Active Living World emits full delta.parallel plus scheduled delta.offscreen changes linked to exact threads. Inline uses no-evidence semantics: spend no tokens on exact quotes; each subplot still needs beatKind, concrete impact, and short grounding, with before/after on later beats. Bonds are signed deltas; knowledge needs a source. Keep under ~{{var::state_reserve_tokens}} tokens.
...
- offscreen: {op,id,name?,who?,where?,gist,beatKind,impact,grounding:{basis:[...],rationale,refs?,before?,after?},thread?,pressure?,hooks?,stakes?,autonomy?,nextTurn?,nextDay?,nextClock?,deadlineDay?,deadlineClock?,dependsOn?,blockedBy?,trigger?}; op=new|advance|resolve. Inline requires semantic grounding, never exact evidence quotes. New rows ground actor/place; later rows include subplot basis and before/after real change. Reuse ids/schedules/threads; exclude present actors; mirror newest who/where/gist exactly in parallel.
