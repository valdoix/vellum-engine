...
const stateProtocol = JSON.parse(fs.readFileSync(path.join(repoRoot, 'src', 'domain', 'state-protocol.json'), 'utf8'));
const stateVersion = stateProtocol.version;
const subplotTitleContract = stateProtocol.subplotTitleContract;
...
const sandboxOn = '{{matches::{{var::living_world}} {{var::social}} {{var::politics}}::(?:^| )(?:sandbox|autonomous)(?: |$)}}';
...
::active::The world does not pause when {{user}} looks away. Absent characters pursue established goals from their own motives, locations, resources, and knowledge. They cannot react to the visible scene until a plausible channel reaches them. Keep every movement synchronized with final T1; an off-screen move may be inferred without visible prose only when an established route, elapsed time, and canon make it feasible.
...
[CAUSAL WORLD PULSE]
{{if::${parallelOn}}}Maintain durable subplots from intent, purpose, or world pressure. Each is a causal commitment: keep id as a stable snake_case machine reference, but give name ${subplotTitleContract}. Require beatKind (progress|obstacle|consequence|bridge|resolution), concrete impact, and semantic grounding; later beats prove prior condition -> changed condition with before/after. For grounding.before on an advance or resolve, paraphrase the subplot's CURRENT status (its last recorded gist or most recent beat), not the world-state before the subplot existed. Evidence strength scales with travel, knowledge, social, and permanence claims. Set realistic schedules; a line may tick consecutively or sleep—never one cadence. Blocked attempts pay a cost and reschedule. Declare locationOp for every durable location decision: retain keeps the prior place and omits a conflicting destination; refine clarifies the same nearby place; move requires a distinct destination, route, and enough time. Living/Active may move at most two eligible rows and open one reversible line.{{if::${sandboxOn}}} Autonomous/Sandbox MUST open at least two new delta.offscreen rows (op:"new") per in-character turn—a hard floor, not a target. Each needs a living NPC actor, canon-plausible place, beatKind, impact, grounding, a stable id, and a creative display name. Derive from canonical motives and world pressure, never narrator knowledge or the persona.{{/if}} Major outcomes need an interruptible chain. One physical truth: newest subplot and parallel place/activity match; foreground intersection shows the actor unless exit, concealment, or interruption is established. Mature exact threads through concrete effects. The persona character never appears here.{{/if}}
...
Build it from canonical T0. Carry each absent actor's prior where/activity/knowledge into T1 unless an actor-specific event or a canon-plausible autonomy decision changes it. Parallel evidence need not appear in visible prose. Ask: is this established character alive; is the destination compatible with last location, route, elapsed time, setting geography, and lore; is the activity in character, feasible, reversible, and knowledge-limited? If yes, record a short rationale. ADVANCE keeps where. MOVE needs enough time and either depicted travel or a canon-grounded route. A distant fact still needs a depicted message, report, call, witness, arrival, or consequence before it can affect that actor. Final present, deceased characters, and the persona character MUST NOT appear in parallel; require where for who; keep one concurrent row per actor at the scene's final day/clock. Preserve uncertain rows and use [] only when none remains. Example: Spike at the Bronze can be plausible within Sunnydale; Spike in Beijing is not plausible without an established route, enough time, and lore support. {{if::${sandboxOn}}}Autonomous/Sandbox originates its required floor from canonical motives and world pressures, never from narrator knowledge or the persona.{{/if}}{{/if}}`, { group: CAT_SIM }),
...
A record changes only when a future turn should behave differently because this turn happened. Evidence is semantic support, not an exact-quote test: accept a factually correct current-scene passage, faithful paraphrase, or relevant lorebook passage when it materially grounds the claim. Merely related or contradictory text fails. Autonomous parallel/off-screen simulation instead uses the explicit life, location, elapsed-time, lore, motive, and knowledge plausibility audit above; it need not be mentioned in visible prose.
...
{v:${stateVersion},turn?,day?,scene?:{title?,transition?:continue|scene|time_skip,loc?,time?,clock?,tension?,weather?},present?:[{id or name,presence?,mood?,doing?,condition?,thought?,traits?,evidence?}],delta?:{bonds?,threads?,arcs?,journal?,knowledge?,secrets?,secretReveals?,factions?,factionRelations?,parallel?,offscreen?},ext?:{scars?,codex?,inventory?,timeline?,intent?,affect?,introduction?,plant?,payoff?}}

Always emit v:${stateVersion}. Active scenes require matching HH:MM/clock, e.g. "time":"07:45","clock":465. Put {{user}} first: blank unless PERSONA STATE is ON; then always populate mood, condition, doing, private first-person thought, and stable traits. List every named on-stage NPC with a concise first-person thought limited to their knowledge. On turn 1 or an empty ledger, create one supported actionable unresolved thread and a clear parent arc; set thread.arc to the exact arc title. Active Living World emits full delta.parallel plus scheduled delta.offscreen changes linked to exact threads. Inline uses no-evidence semantics: spend no tokens on exact quotes; each subplot still needs beatKind, concrete impact, short grounding, a stable machine id, and name as ${subplotTitleContract}, with before/after on later beats (grounding.before paraphrases the subplot's current status, not the pre-subplot world-state). Use locationOp retain|refine|move explicitly. Bonds are signed deltas; knowledge needs a source. Keep under ~{{var::state_reserve_tokens}} tokens.
...
SUPPORTED TOP LEVEL:
- v: ${stateVersion} — required protocol version; always emit exactly this value
...
- delta?: {bonds?, threads?, arcs?, journal?, knowledge?, secrets?, secretReveals?, factions?, factionRelations?, parallel?, offscreen?}
...
- secret reveal: {id:"exact prior secret id",to:[names]}.
...
- faction relation: {a,b,kind?,standing?,absolute?,why?}. kind is alliance|rivalry|war|vassal|trade. standing is a small signed change. absolute is reserved for explicit establishment/correction; omit it in ordinary narration.
  - parallel: {who?,where?,activity,note?}. Complete replace-all T1 snapshot. Preserve prior actor rows. A new autonomous beat needs a living established NPC, canon-plausible location/activity, sufficient time and route for movement, and limited knowledge—not necessarily a prose quote. Exclude final present and deceased actors, require where for who, keep one row per actor, and use [] only when all prior rows resolve.
- offscreen: {op,id,name,who?,where?,locationOp?:retain|refine|move,gist,beatKind,impact,grounding:{basis:[...],rationale,refs?,before?,after?},thread?,arc?,pressure?,hooks?,stakes?,autonomy?,nextTurn?,nextDay?,nextClock?,deadlineDay?,deadlineClock?,dependsOn?,blockedBy?,trigger?}; op=new|advance|resolve. Keep id stable and snake_case; name is ${subplotTitleContract}. Existing updates reuse the exact id and display name. Inline requires semantic grounding, never exact evidence quotes. New rows ground actor/place; later rows include subplot basis and before/after real change (grounding.before paraphrases the subplot's current gist or most recent beat, not the pre-subplot world-state). locationOp retain keeps the prior place, refine clarifies the same nearby place, and move requires a distinct feasible destination. Reuse ids/schedules/threads; exclude present actors; mirror newest who/where/gist exactly in parallel.
...
<vellum>
{"v":${stateVersion},"turn":6,"day":0,"scene":{"title":"The Letter","transition":"continue","loc":"Observatory","time":"03:05","clock":185},"present":[{"id":"{{user}}","thought":""},{"id":"Mira","thought":"Why wait?"}],"delta":{"bonds":[{"a":"{{user}}","b":"Mira","addCats":["alliance"]}],"arcs":[{"op":"new","name":"The Observatory Mystery","note":"A sealed letter appears in the observatory."}],"threads":[{"op":"new","name":"The Sealed Letter","note":"A sealed letter appears in the observatory.","arc":"The Observatory Mystery"}]},"ext":{}}
</vellum>
...
T — Truthful deltas: {{if::${inlineState}}}enumerate exact supported state sections and signed changes; reconcile present and parallel at T1{{else}}{{if::${engineState}}}name grounded foreground evidence, plus any separate canon-plausible parallel activity, and omit unsupported changes{{else}}name continuity facts to preserve with state disabled{{/if}}{{/if}}.
...
1. CORE SNAPSHOT: begin the object with "v":${stateVersion}, then write current scene/present. Require exact HH:MM and matching clock; narrative periods are invalid. Compute A0/A1 as day × 1440 + clock; if A1 < A0, retain T0 or recompute from established elapsed time. Put {{user}} first. PERSONA STATE ON requires a complete tracker snapshot regardless of agency; this metadata never licenses prose behavior.
...
4. PARALLEL RECONCILIATION: freeze final T1 after the prose. Remove every parallel item whose actor is present or deceased. Preserve prior rows, then accept new autonomy without a prose quote only for a living established actor at a canon-plausible location with feasible elapsed travel, fitting activity, and limited knowledge. Require who+where, allow one item per actor, and emit [] rather than stale or implausible content. For durable offscreen rows, reuse the stable id and exact existing display name; new names must be ${subplotTitleContract}. Declare locationOp as retain, refine, or move and obey its travel semantics.
...
Final audit: protocol v${stateVersion}; exact names; {{user}} first, blank with PERSONA STATE OFF and complete in every agency when ON; each on-stage NPC has a limited-knowledge thought; no off-scene knowledge leak; no present actor or stale location in parallel; exact matching time/clock; signed changes; addCats/removeCats; sourced knowledge; exact plot names; stable subplot ids; creative Title Case subplot display names; valid locationOp intent; supported fields only; valid JSON; closed </vellum>; nothing after.{{/if}}`, { group: CAT_FINAL, position: 'post_history' }),
...
Reserve ~{{var::state_reserve_tokens}} output tokens, precompose the full object beginning with "v":${stateVersion}, then emit balanced raw JSON and literal </vellum>. Shorten prose before risking state. A reply ending anywhere else is incomplete.{{/if}}
...
for (const requiredTerm of [`v:${stateVersion}`, 'addCats', 'removeCats', 'arcs?', 'secretReveals?', 'factionRelations?', 'offscreen?', 'locationOp?', 'inventory?', 'intent?', 'affect?', 'introduction?', 'plant?', 'payoff?']) {
...
assert(stateFinalBlock.includes('PARALLEL RECONCILIATION') && stateFinalBlock.includes('emit [] rather than stale or implausible content'), 'Final state compiler lacks parallel reconciliation');
...
assert(outputContractBlock.includes('before/after proof of actual change') && outputContractBlock.includes('subplot and parallel place/activity must match'), 'Last-instruction subplot consistency gate missing');
...
preset.metadata = { vellum_engine: { identifier: 'vellum_engine', policyCompiler: 1, stateProtocolVersion: stateVersion, sourceHash: createHash('sha256').update(fs.readFileSync(fileURLToPath(import.meta.url))).digest('hex'), baseHash: createHash('sha256').update(fs.readFileSync(basePath)).digest('hex') } };
