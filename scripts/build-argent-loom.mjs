...
[CAUSAL WORLD PULSE]
{{if::${parallelOn}}}Maintain durable subplots from intent, purpose, or world pressure. Set a realistic nextTurn, day/clock, or trigger; a line may tick consecutively or sleep—never one cadence. Require logistics and knowledge; blocked attempts pay a cost and reschedule. Living/Active may move at most two eligible rows and open at most one reversible line. Autonomous/Sandbox must create at least four parallel events and at least two new independent durable subplots on every in-character pulse; it has no narrative maximum, so add more whenever canon, motive, logistics, time, and knowledge support them. Major outcomes need an interruptible chain. Always preserve the current parallel snapshot. Mature exact threads through a message, clue, arrival, absence, faction move, or material effect. The persona character never appears in a subplot or parallel event.{{/if}}
...
Build it from canonical T0. Carry each absent actor's prior where/activity/knowledge into T1 unless an actor-specific clause changes it. ADVANCE keeps where; MOVE needs depicted travel, an established destination, and enough time. A distant fact needs a depicted message, report, call, witness, arrival, or consequence before it can affect that actor. Final present and the persona character MUST NOT appear in parallel; require where for who; keep one concurrent row per actor at the scene's final day/clock. Preserve uncertain rows and use [] only when none remains. Living/Active never invents a beat to fill parallel. {{if::${sandboxOn}}}Autonomous/Sandbox originates its required floor from canonical motives and world pressures, never from narrator knowledge or the persona.{{/if}}{{/if}}`, { group: CAT_SIM }),
...
The engine may originate grounded off-screen subplots without ((parallel)). In Inline Compatibility serialize them in delta.offscreen and mirror current activity in delta.parallel; in Engine Second Pass compile them separately. Move one only when due. Living/Active may handle at most two due rows and one new reversible line. Autonomous/Sandbox must create at least four parallel events and at least two new independent durable subplots on every in-character pass and has no narrative maximum. The current snapshot must remain visible when nothing is due. Social/Politics still control the kinds and scale of change; never author player choices, consent, acts, or any subplot/parallel role for the persona character.{{/if}}
